...
class ActionItemIframe extends ActionItem
{
    public $containerId;  // div contenant l’iframe
    public $iframeId;     // id de l’iframe
    public $iframePage;   // page à charger dans l’iframe
     ...
     * Display principal
     */
    public function display($param, $extraParams = [])
    {
        // Inclure le JS une seule fois (optionnel si déjà dans main.php)
        // if (!self::$jsIncluded) {
        //     $this->includeJS();
     ...
     */
    private function hasCorrectAcl(): bool
    {
        if (function_exists("hasCorrectAcl")) {
            return hasCorrectAcl($this->module, $this->submod, $this->action);
        }
        return true;
    }

    /**
     * Comportement avec droits OK
     */
    public function displayWithRight($param, $extraParams = [])
    {
        $jsonParams = htmlspecialchars(json_encode($extraParams), ENT_QUOTES);
        $ajaxUrl = "main.php?action={$this->action}&module={$this->module}&submod={$this->submod}";

        echo "<li class=\"{$this->classCss}\">";
        echo "<a title=\"{$this->desc}\" onclick=\"toggleIframeAndSendParams(
                '{$this->containerId}',
                '{$this->iframeId}',
                {$jsonParams},
                '{$ajaxUrl}'
    ...
    /**
     * Comportement sans droits
     */
    public function displayWithNoRight($param, $extraParams = [])
    {
        echo "<li class=\"{$this->classCss}\" style=\"opacity:0.3;\">";
        echo "<a title=\"{$this->desc}\" href=\"#\" onclick=\"return false;\">&nbsp;</a>";
        echo "</li>";
    }

    /**
     * Méthode encapsulate() appelée par PageGenerator
     */
    public function encapsulate($obj, $extraParams = [])
    {
        $jsonParams = htmlspecialchars(json_encode($extraParams), ENT_QUOTES);
        $ajaxUrl    = "main.php?action={$this->action}&module={$this->module}&submod={$this->submod}";

        $html  = "<a title=\"{$this->desc}\" onclick=\"toggleIframeAndSendParams(";
        $html .= "'{$this->containerId}', '{$this->iframeId}', {$jsonParams}, '{$ajaxUrl}'";
        $html .= ")\">";
        $html .= htmlspecialchars($obj);
        $html .= "</a>";
    ...
    /**
     * Inclusion JS (optionnelle si déjà inclus dans main.php)
     */
...
function toggleIframeAndSendParams(containerId, iframeId, params, url) {
    ...
    for (let key in params) {
        ajaxUrl += "&" + encodeURIComponent(key) + "=" + encodeURIComponent(params[key]);
    }
