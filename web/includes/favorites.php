...
foreach(getSorted($LMCApp->getModules()) as $mkey => $mod) {
    ...
    foreach($sort_a[$mod->getName()] as $key => $al) {
        ...
        foreach($al as $akey => $value) {
            ...
            $getrecop['uri']=$uri;
            ?> <span class="redbutton"><a href="#" onClick="new Ajax.Updater('__popup_container','<?= urlStr('base/main/favorites',$getrecop); ?>'); return false">X</a></span> <?
...
if ($local_page->isVisible()) {
    ...
    <p><a href="#" onClick="new Ajax.Updater('__popup_container','<?= urlStr('base/main/favorites',$_GET); ?>'); return false"><?= _("Add this page to your favorite") ?></a></p>
    <?
}
