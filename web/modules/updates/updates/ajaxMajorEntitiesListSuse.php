...
// file : web/modules/updates/updates/ajaxMajorEntitiesListSuse.php
/*
 ...
 * file: ajaxMajorEntitiesListSuse.php
 */
 ...
 * ------------------------------------------------------------------
 */
$distribution = "suse"; // Distribution analysée
...
foreach ($statversion['by_entity'] as $entityId => $ent) {
    ...
    if (intval($ent['pending_support_update']) > 0) {
        ...
        $msgtitle = sprintf(
            "%s\n\n%s\n%s",
            _T("Machines with SUSE version pending support update", "updates"),
            _T("Create group:", "updates"),
            $namegrp
        );
    ...
    if (intval($ent['total_machines']) > 0) {
        ...
        $msgtitle = sprintf(
            "%s %s\n\n%s\n%s",
            _T("Total SUSE machines in entity", "updates"),
            $entityName,
            _T("Create group:", "updates"),
            $namegrp
        );
