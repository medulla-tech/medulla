...
// file : web/modules/updates/updates/ajaxMajorDeploymentHistoryWin.php
...
if (!$hasEntity || $entityId < 0) {
    echo '<div class="error">' . _T("Missing entity for Windows major deployment history.", "updates") . '</div>';
    return;
}
...
$result = xmlrpc_get_win_major_deployment_history_by_entity($entityId, $start, $limit, $filter);
if (!is_array($result)) {
    $result = array();
}
...
$n->setNavBar(new AjaxNavBar($count, $filter, "updateSearchParamMajorDeploymentHistoryWin"));
$n->setParamInfo($params);
$n->setEmptyState(_T("No deployment history", "updates"), _T("No Windows major deployments have been recorded for this entity in the last month.", "updates"));
$n->startreal = $start;
$n->endreal   = $end;
$n->start = 0;
$n->end   = count($titles) - 1;

echo '<h2>' . _T("Past Windows major deployments (last month)", "updates") . '</h2>';
