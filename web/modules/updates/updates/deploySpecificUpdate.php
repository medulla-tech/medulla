...
if(!empty($_GET['entity'])) {
    ...
    $p = new PageGenerator(sprintf(_T("Updates on Entity %s", "updates"), $completename));
} elseif(!empty($_GET['gid'])) {
    $gid = htmlentities($_GET['gid']);
    $groupname = htmlentities($_GET['groupname']);
    $params = ["group" => $gid, "groupname" => $groupname];

    $p = new PageGenerator(sprintf(_T("Updates on Group %s", "updates"), $groupname));
    ...
    $p = new PageGenerator(sprintf(_T("Updates on machine %s", "updates"), $cn));
} else{
