...
// file : web/modules/updates/updates/ajaxMajorEntitiesListAlma.php
/*
 ...
 * file: ajaxMajorEntitiesListAlma.php
 */
 ...
 * ------------------------------------------------------------------
 */
$distribution = "almalinux"; // Distribution analysée
...
foreach ($statversion['by_entity'] as $entityId => $ent) {
    ...
    if (intval($ent['pending_support_update']) > 0) {
        ...
        $msgtitle = sprintf(
            "%s\n\n%s\n%s",
            _T("Machines with AlmaLinux version pending support update", "updates"),
            _T("Create group:", "updates"),
            $namegrp
        );
    ...
    if (intval($ent['total_machines']) > 0) {
        ...
        $msgtitle = sprintf(
            "%s %s\n\n%s\n%s",
            _T("Total AlmaLinux machines in entity", "updates"),
            $entityName,
            _T("Create group:", "updates"),
            $namegrp
        );
