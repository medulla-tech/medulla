...
// file : web/modules/updates/updates/ajaxMajorEntitiesListFedora.php
/*
 ...
 * file: ajaxMajorEntitiesListFedora.php
 */
 ...
 * ------------------------------------------------------------------
 */
$distribution = "fedora"; // Distribution analysée
...
foreach ($statversion['by_entity'] as $entityId => $ent) {
    ...
    if (intval($ent['pending_support_update']) > 0) {
        ...
        $msgtitle = sprintf(
            "%s\n\n%s\n%s",
            _T("Machines with Fedora version pending support update", "updates"),
            _T("Create group:", "updates"),
            $namegrp
        );
    ...
    if (intval($ent['total_machines']) > 0) {
        ...
        $msgtitle = sprintf(
            "%s %s\n\n%s\n%s",
            _T("Total Fedora machines in entity", "updates"),
            $entityName,
            _T("Create group:", "updates"),
            $namegrp
        );
