...
if (isset($_POST["bcreate"]) || isset($_POST["bassoc"])) {
    ...
    if (!isXMLRPCError() and $ret and $ret != -1) {
        if ($ret[0]) {
            ...
            if (isset($_POST["bassoc"])) {
                header("Location: " . urlStrRedirect("pkgs/pkgs/associate_files_samba", array('p_api'=>base64_encode($p_api_id), 'pid'=>base64_encode($ret[3]['id']), 'plabel'=>base64_encode($ret[3]['label']), 'pversion'=>base64_encode($ret[3]['version']), 'mode'=>$_POST['mode'])));
            }
        } else {
