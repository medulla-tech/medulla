...
foreach ($devices as $d) {
    if (isset($d['id']) && (int)$d['id'] === $deviceId) { $device = $d; break; }
}

if (!$device) {
    echo json_encode(['ok' => false, 'error' => _T("Device not found", "mobile")]);
    exit;
}

$email = trim($device['custom1'] ?? '');
$name  = trim($device['number'] ?? '');

if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    echo json_encode(['ok' => false, 'error' => _T("No email address configured for this device. Edit the device to add one.", "mobile")]);
    exit;
}

$qr_url = xmlrpc_get_hmdm_config_qr_url();
if (!$qr_url) {
    echo json_encode(['ok' => false, 'error' => _T("Could not retrieve enrollment QR code", "mobile")]);
    exit;
}
...
echo json_encode([
    ...
    'name'  => $name,
    'email' => $email,
    'error' => $ok ? null : _T("Failed to send email", "mobile")
]);
