...
$value = implode('/', [trim($basename, '/'), trim($forward, '/'), trim($file['name'], '/') ]);
    ...
    $creations[] = date("Y-m-d H:i:s", $file["creat"]);
    // $modifications[] = secs2date($file['mod']);
    $modifications[] = date("Y-m-d H:i:s", $file['mod']);
    // $access[] = secs2date($file['access']);
    $access[] = date("Y-m-d H:i:s", $file['access']);
...
echo '<p>';
echo sprintf('<a title="" onclick="PopupWindow(event,\'main.php?module=urbackup&amp;submod=urbackup&amp;action=basket\', 0); return false;" href="main.php?module=urackup&amp;submod=urbackup&amp;action=basket" id="view-basket">'._T("Basket (<span id='total-in-basket'>%s</span> elements selected)", "urbackup").'</a><br>', $count_total_basket);
