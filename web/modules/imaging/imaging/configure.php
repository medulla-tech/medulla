...
function expertModeDisplay($f, $has_profile, $type, $menu, $opts, $target, $real_target)
{
    if (!$has_profile) {
        ...
        foreach($opts as $key => $str) {
            ...
            $f->add(
                new TrFormElement($str, new CheckBoxTpl($key)),
                array("value" => $value)
            );
        }
        $f->pop();
        ...
        $f->add(
            new TrFormElement(
                _T("Full path to the XPM displayed at boot", "imaging"),
                new InputTpl("boot_xpm")
            ),
            array("value" => $menu['background_uri'])
        );
        $f->add(
            new TrFormElement(
                _T("Message displayed during backup/restoration", "imaging"),
                new TextareaTpl("boot_msg")
            ),
    ...
    if ($type == '') {
        ...
        $f->add(
            new TrFormElement(
                _T("Kernel parameters", "imaging"),
                new InputTpl("target_opt_kernel")
            ),
            array("value" => $local_kernel_parameters)
        );
        $f->add(
            new TrFormElement(
                _T("Image parameters", "imaging"),
                new InputTpl("target_opt_image")
            ),
            array("value" => $local_image_parameters)
        );

        $f->add(
            new TrFormElement(
                _T("Force raw backup mode", "imaging"),
...
if (isset($_POST["bvalid"])) {
    ...
    if ($type == 'group') { // profile
        xmlrpc_setfromxmppmasterlogxmpp(
            sprintf(_T("choose network profile for group %s", "imaging"), urldecode($label), $target_name),
            "IMG",
            '',
            0,
            $target_name,
        ...
        xmlrpc_setfromxmppmasterlogxmpp(
            sprintf(_T("choose network profile for machine %s", "imaging"), urldecode($label), $target_name),
            "IMG",
            '',
            0,
            $target_name,
...
// Display register page form
    ...
    if(is_string($whose) && strtolower($whose) == 'false') {
        $whose = false;
    } elseif(is_string($whose) && strtolower($whose) == 'true') {
        $whose = true;
    }
    ...
    if (!$whose && !$menu) {
        ...
        $f->add(new TitleElement($msg, 3));
        $f->display();
    } elseif (($type == '') && ($is_registering) && (xmlrpc_checkComputerForImaging($target_uuid) != 0)) {
        ...
        $f->add(new TitleElement($msg, 3));
        $f->display();
    } elseif (($type == 'group') && ($is_registering) && ($checkProfileForImagingStatus = xmlrpc_checkProfileForImaging($target_uuid) and $checkProfileForImagingStatus != 0)) {
            ...
            foreach ($profileNetworks as $networks) {
                ...
                if (is_array($networks) && safeCount($networks) > 1 and isset($networks['macAddress'])) {
                    if (safeCount($networks['macAddress']) > 1) {
                        ...
                        $f->add(
                            new HiddenTpl("choose_network_" . $networks['objectUUID'][0]),
                            array(
        ...
        if (($type == '') && (!$is_registering) && (!$is_unregistering)) {
            ...
            foreach($inventory as $disk => $parts) {
                ...
                $f->push(new Table());
                $f->add(
                    new TrFormElement(_T("Select this hard disk", "imaging"), new CheckboxTpl($inputvar)),
                    array("value" => $value,
