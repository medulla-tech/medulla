...
?>
    <tr class="acl-row-category">
        ...
        <td class="acl-td-center">
            <a href="#" onclick="toggleCategory('<?php echo $cat; ?>', '<?php echo $profile; ?>', true); return false;"
               class="acl-toggle-link" title="<?php echo _T("Check all", "admin"); ?>"><?php echo _T("all", "admin"); ?></a>
            <span class="acl-toggle-sep"> | </span>
            <a href="#" onclick="toggleCategory('<?php echo $cat; ?>', '<?php echo $profile; ?>', false); return false;"
               class="acl-toggle-link" title="<?php echo _T("Uncheck all", "admin"); ?>"><?php echo _T("no one", "admin"); ?></a>
        </td>
        <?php endforeach; ?>
    </tr>
    ...
    <tr<?php echo $altClass; ?>>
        ...
        <?php foreach ($profiles as $profile):
            ...
            $disabled = ($isSuperadminOnly && $profile !== 'Super-Admin') ? ' disabled' : '';
            if ($lockedThis) { $checked = ' checked'; $disabled = ' disabled'; }
        ?>
        <td class="acl-td-center">
            <?php if (!$isSuperadminOnly || $profile === 'Super-Admin'): ?>
            <?php if ($lockedThis): ?>
...
function toggleCategory(category, profile, state) {
    ...
    boxes.forEach(function(box) { box.checked = state; });
}
