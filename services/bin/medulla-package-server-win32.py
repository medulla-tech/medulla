...
class Medulla2PackageServer(win32serviceutil.ServiceFramework):
    _svc_name_ = "Medulla2PackageServer"
    _svc_display_name_ = "Medulla 2 Package Server"
    ...
    def init(self, config):
        from medulla.package_server import (
            ThreadLauncher,
            init_logger_debug,
            getRevision,
            getVersion,
        ...
        init_logger_debug()
        logger.info(f"Medulla 2 Package Server {getVersion()} starting...")
        logger.info(f"Medulla 2 Package Server build '{str(getRevision())}'")
...
if __name__ == "__main__":
    win32serviceutil.HandleCommandLine(Medulla2PackageServer)
