...
check_medulla_version() {
    ...
    AVAILABLE_VERSION=$(LC_ALL=C apt-cache policy pulse2-common | grep Candidate | awk '{print $2}' | cut -d'g' -f1)
}
...
update_550_to_551() {
    str="Applying Medulla config update from 5.5.0 to 5.5.1..."
    echo "$str"
    write_to_log "$str"
    update_medulla
...
update_562_to_563() {
    str="Applying Medulla config update from 5.6.2 to 5.6.3..."
    echo "$str"
    write_to_log "$str"
    update_medulla

    echo "5.6.3" > /var/lib/mmc/version
    str="[v] Medulla config update from 5.6.2 to 5.6.3 applied successfully."
    echo "$str"
    write_to_log "$str"
    if [[ -f /tmp/update_medulla.sh ]]; then
        exec /tmp/update_medulla.sh "$@"
...
update_564_to_565() {
    ...
    str="[=] Hardening reversessh connection..."
    ...
    str="[v] Hardening reversessh connection completed successfully."
    echo "$str"
    write_to_log "$str"

    # Install prerequisites for generating Mac agents
