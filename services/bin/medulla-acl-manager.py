...
# ============================================================================
# Export command - dump acl_feature_definitions as a replay-safe SQL file
# ============================================================================

def _sql_quote(value):
    """SQL-quote a value for inclusion in a generated INSERT statement."""
    if value is None:
        return "NULL"
...
def cmd_features_export(output_file, version=None):
    """Dump acl_feature_definitions to a replay-safe SQL file.

    Only acl_feature_definitions is exported. acl_profile_features (per-profile
    customisations) is left untouched on the client side because it holds the
    customer's own ACL choices, not feature definitions.
    """
    import datetime, socket

    rows = query(
        "SELECT feature_key, label, description, category, superadmin_only, "
        "acl_entry, access_type, install_types "
        "FROM acl_feature_definitions "
        "ORDER BY category, feature_key, access_type, id"
    )
    if not rows:
        print(f"{RED}No feature definitions to export.{NC}")
        return

    lines = []
    lines.append("--")
    lines.append(f"-- Auto-generated by medulla-acl-manager export")
    lines.append(f"-- Generated: {datetime.datetime.now().isoformat(timespec='seconds')} from {socket.gethostname()}")
    lines.append(f"-- {len(rows)} ACL entries across "
                 f"{len(set(r['feature_key'] for r in rows))} features")
    lines.append("--")
    lines.append("-- Replaces the full content of acl_feature_definitions with the")
    lines.append("-- definitions exported from the dev installation. Customer")
    lines.append("-- customisations of acl_profile_features are NOT affected.")
    lines.append("--")
    lines.append("")
    ...
    lines.append("")
    lines.append("DELETE FROM acl_feature_definitions;")
    lines.append("")

    # Group by category then by feature_key for readability; one INSERT per
    # feature_key (multi-rows) so a feature's ACL entries stay together.
    ...
    def flush():
        if values_buffer:
            lines.append("INSERT INTO acl_feature_definitions "
                         "(feature_key, label, description, category, superadmin_only, "
                         "acl_entry, access_type, install_types) VALUES")
            lines.append(",\n".join("  " + v for v in values_buffer) + ";")
            lines.append("")
            values_buffer.clear()
    ...
    for r in rows:
        if r["category"] != current_category:
            ...
            lines.append("")
        if r["feature_key"] != current_feature:
            flush()
            current_feature = r["feature_key"]
        values_buffer.append(
            "(" + ", ".join([
                _sql_quote(r["feature_key"]),
                _sql_quote(r["label"]),
                _sql_quote(r["description"]),
                _sql_quote(r["category"]),
                _sql_quote(bool(r["superadmin_only"])),
                _sql_quote(r["acl_entry"]),
                _sql_quote(r["access_type"]),
                _sql_quote(r["install_types"]),
            ]) + ")"
        )
    flush()

    if version is not None:
        lines.append(f"UPDATE version SET Number = {int(version)};")
        lines.append("")
    ...
    feature_count = len(set(r["feature_key"] for r in rows))
    print(f"{GREEN}Exported {len(rows)} ACL entries across {feature_count} features "
          f"to {output_file}{NC}")
    if version is not None:
        print(f"  Schema version will be set to {version} when this file is applied.")
...
{BOLD}Usage:{NC}
  ...
  medulla-acl-manager features export <file.sql> [--version N]
                                                          Dump feature definitions
                                                          as a replay-safe SQL file
...
{BOLD}Examples:{NC}
  ...
  medulla-acl-manager features export /tmp/acl_features.sql --version 17
...
def main():
    ...
    if cmd == "profiles":
        ...
        if sub == "list":
            ...
            # features export <output.sql> [--version N]
            version = None
            if "--version" in args:
                vi = args.index("--version")
                if vi + 1 < len(args):
                    try:
                        version = int(args[vi + 1])
                    except ValueError:
                        print(f"{RED}--version expects an integer.{NC}")
                        sys.exit(1)
            cmd_features_export(args[2], version=version)
        else:
            usage()
            sys.exit(1)
