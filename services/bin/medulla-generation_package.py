...
if __name__ == "__main__":
    ...
    Ce Programme permet de migrer
        les packages de /var/lib/medulla/packages
        ...
        vers
        les partages /var/lib/medulla/packages/sharing
    ...
    optp.add_option(
        ...
        dest="movebadpackage",
        default=False,
        help="deplace les packages avec erreur vers /var/lib/medulla/packageerror",
    )
    ...
    if not os.path.exists("/var/lib/medulla/packageerror"):
        os.mkdir("/var/lib/medulla/packageerror")
    ...
    try:
            ...
            for path_package in package_partage[partage]:
                ...
                sql = """INSERT INTO `pkgs`.`packages` (
                                                ...
                                                VALUES ("%s","%s","%s","%s","%s",
                                                        ...
                                                        "%s","%s","%s","%s");""" % (
                    fiche["label"].decode(),
                    fiche["description"].decode(),
                    fiche["uuid"].decode(),
                    fiche["version"].decode(),
                    fiche["os"].decode(),
                    fiche["metagenerator"].decode(),
                    fiche["entity_id"].decode(),
                    fiche["sub_packages"].decode(),
                    fiche["reboot"].decode(),
                    fiche["inventory_associateinventory"].decode(),
                    fiche["inventory_licenses"].decode(),
                    fiche["Qversion"].decode(),
                    fiche["Qvendor"].decode(),
                    fiche["Qsoftware"].decode(),
                    fiche["boolcnd"].decode(),
                    fiche["postCommandSuccess_command"].decode(),
                    fiche["postCommandSuccess_name"].decode(),
                    fiche["installInit_command"].decode(),
                    fiche["installInit_name"].decode(),
                    fiche["postCommandFailure_command"].decode(),
                    fiche["postCommandFailure_name"].decode(),
                    fiche["command_command"].decode(),
                    fiche["command_name"].decode(),
                    fiche["preCommand_command"].decode(),
                    fiche["preCommand_name"].decode(),
                    fiche["pkgs_share_id"].decode(),
                    fiche["edition_status"].decode(),
                    fiche["conf_json"].decode(),
                    fiche["size"].decode(),
                )
                # print(sql)
                try:
        ...
        if opts.movebadpackage:
            for t in bad_package:
                ...
                re = simplecommand("mv %s /var/lib/medulla/packageerror/" % t)
                if opts.verbeux:
                    if re["code"] != 0:
                        print(re["result"])
