...
from pulse2.consts import (
    ...
    PULSE2_PSERVER_FMIRRORFAILED_404_ERROR,
)
from pulse2.consts import (
    PULSE2_PSERVER_FMIRRORFAILED_CONNREF_ERROR,
    PULSE2_PSERVER_GETFILEURIFROMPACKAGE_ERROR,
)
from pulse2.consts import (
    PULSE2_PSERVER_GETFILEURIFROMPACKAGE_F_ERROR,
    PULSE2_PSERVER_ISAVAILABLE_FALLBACK,
)
from pulse2.consts import (
    PULSE2_PSERVER_ISAVAILABLE_MIRROR,
    PULSE2_PSERVER_MIRRORFAILED_404_ERROR,
)
from pulse2.consts import (
...
class RemoteControlPhase(Phase):
    ...
    def _perform(self):
        ...
        if not client["host"]:  # We couldn't get an IP address for the target host
            fd = defer.fail(
                Exception(
                    "Not enough information about client to perform %s" % self.name
                )
            )
            fd.addErrback(
                self.parse_remote_phase_error,
    ...
    def parse_remote_phase_error(self, reason, error_code=PULSE2_UNKNOWN_ERROR):
        """
        ...
        self.logger.warn(
            "Circuit #%s: (%s on %s) %s failed, unattented reason: %s"
            % (self.coh.id, self.cmd.title, self.target.target_name, self.name, reason)
        )
...
class WOLPhase(Phase):
    ...
    def perform(self):
        def _cb(result):
            ...
            if result == 2:
                self.logger.info(
                    "Circuit #%s: do not wol (target already up)" % self.coh.id
                )
                ...
                return next(self)
            self.logger.info(
                "Circuit #%s: (%s on %s) do wol (target not up)"
                % (self.coh.id, self.cmd.title, self.target.target_name)
            )
    ...
    def parseWOLAttempt(self, attempt_result):
        ...
        try:
            ...
            self.logger.error(
                "Circuit #%s: WOL request seems to have failed ?!" % (self.coh.id)
            )
            if not self.coh.isStateStopped():
                self.coh.setStateScheduled()
...
class UploadPhase(RemoteControlPhase):
    ...
    def _cbChooseUploadMode(self, result):
        if result == "waiting":
            self.logger.info(
                "Circuit #%s: waiting for a local proxy" % self.coh.getId()
            )
            # TODO
            if not self.coh.isStateStopped():
                self.coh.setStateScheduled()
            ...
            self.logger.info(
                "Circuit #%s: becoming local proxy server" % self.coh.getId()
            )
            ...
            self.logger.info(
                "Circuit #%s: becoming local proxy client" % self.coh.getId()
            )
    ...
    def _chooseUploadMode(self):
        ...
        if not client["host"]:  # We couldn't get an IP address for the target host
            ...
            err.addErrback(
                self.parsePushError, error_code=PULSE2_TARGET_NOTENOUGHINFO_ERROR
            )
        ...
        if self.coh.isProxyClient():
            ...
            try:
                ...
                err = defer.fail(
                    Exception("Mirror uri %s is not well-formed" % self.target.mirrors)
                )
            ...
            try:
                ...
                d.addCallback(
                    self._cbRunPushPullPhaseTestMainMirror, mirror, fbmirror, client
                )
    ...
    def _eb_mirror_check(self, failure):
        if hasattr(failure, "trap"):
            err = failure.trap(TimeoutError, ConnectionRefusedError, ConnectionLost)
            if err == TimeoutError:
    ...
    def _runProxyClientPhase(self, client):
        ...
        if proxyCoH is None:
            return (
                defer.fail(Exception("Cant access to CoH"))
        ...
        proxy = {
            "host": chooseClientInfo(proxyCoH.target),
            ...
            "group": "",
        }  # TODO - get from launchers select

        if not proxy["host"]:  # We couldn't get an IP address for the target host
            return (
                defer.fail(Exception("Can't get proxy IP address"))
                .addErrback(self.parsePushError)
                .addErrback(self.got_error_in_error)
            )
        # and fill struct
        ...
        client["proxy"] = {
            "command_id": self.coh.getUsedProxy(),
            "host": proxy["host"],
            "uuid": proxy["uuid"],
        }
        ...
        if self.config.mode == "sync":
            ...
            mydeffered.addCallback(self.parsePushResult).addErrback(
                self.parsePushError
            ).addErrback(self.got_error_in_error)
        elif self.config.mode == "async":
            ...
            mydeffered.addCallback(self.parsePushOrder).addErrback(
                self.parsePushError
            ).addErrback(self.got_error_in_error)
        else:
            mydeffered = None
    ...
    def _runPushPhase(self, client):
        ...
        for file in self.cmd.files.split("\n"):
            ...
            files_list.append(
                os.path.join(
                    re_file_prot_path.search(self.target.mirrors).group(1), fname
                )
            )  # get folder on mirror
    ...
    def _runPushPullPhase(self, mirror, fbmirror, client, useFallback=False):
        ...
        d.addCallback(
            self._cbRunPushPullPhasePushPull, mirror, fbmirror, client, useFallback
        )

        return d

    def _cbRunPushPullPhasePushPull(
        self, result, mirror, fbmirror, client, useFallback
    ):
        if isinstance(result, list) and result[0] == "PULSE2_ERR":
            if result[1] == PULSE2_ERR_CONN_REF:
                self.update_history_failed(
        ...
        if self.config.mode == "sync":
            ...
            mydeffered.addCallback(self.parsePullResult).addErrback(
                self.parsePullError
            ).addErrback(self.got_error_in_error)
        elif self.config.mode == "async":
            mydeffered = self.launchers_provider.async_remote_pull(
            ...
            mydeffered.addCallback(self.parsePullOrder).addErrback(
                self.parsePullError
            ).addErrback(self.got_error_in_error)
        else:
            return self.give_up()
    ...
    def parsePullError(self, reason, error_code=PULSE2_UNKNOWN_ERROR):
        ...
        # something goes really wrong: immediately give up
        self.logger.warn(
            "Circuit #%s: pull failed, unattented reason: %s"
            % (self.coh.id, reason.getErrorMessage())
        )
...
class RebootPhase(RemoteControlPhase):
    ...
    def apply_initial_rules(self):
        ...
        if self.cmd.isPartOfABundle() and not self.dispatcher.bundles.is_last(
            self.coh.id
        ):
    ...
    def perform(self):
        ...
        if output:
            ...
            try:
                ...
                self.logger.warn(
                    "Circuit #%s: Cannot parse WU output: %s" % (self.coh.id, str(e))
                )
                self.update_history_failed(1, "Cannot parse WU output", str(e))
                self.logger.warn(
                    "Circuit #%s: WU Parse phase failed and skipped" % (self.coh.id)
                )
            ...
            if "content" in parsed:
                ...
                wu.purge_obselete_updates(
                    self.target.target_uuid.replace("UUID", ""), update_uuids
                )
                ...
                for line in content:
                    try:
                        ...
                        wu.inject(
                            self.target.target_uuid.replace("UUID", ""),
                            uuid,
                            title,
                            kb_number,
                            kb_type,
        ...
        self.logger.info(
            "Circuit #%s: WU Parse phase successfully processed" % (self.coh.id)
        )
