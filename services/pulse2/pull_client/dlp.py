...
class DlpClient(HTTPClient):
    ...
    def auth(self):
        ...
        try:
            res = self.post(
                "auth",
                {
                    "authkey": self.config.Dlp.authkey,
            ...
            if res.code == 200:
                ...
                logger.error(
                    "Failed to authenticate against the DLP (error %i)" % res.code
                )
        return False
    ...
    def get_commands(self):
        try:
            ...
            if res.code == 200:
                ...
                logger.error(
                    "Failed to get commands from the DLP (error %i)" % res.code
                )
        return []
    ...
    def get_package(self, uuid, workdir):
        ...
        try:
            ...
            if res.code == 200:
                ...
                logger.error(
                    "Failed to get package from the DLP (error: %i)" % res.code
                )
        return False
    ...
    def send_result(self, result):
        ...
        try:
            ...
            res = self.post(
                "step/%s/%s" % (result.command_id, result.step_name), data=data
            )
        except urllib.error.URLError:
            logger.error("Failed to contact the DLP.")
        except http.client.InvalidURL as error:
    ...
    def send_inventory(self, inventory):
        ...
        try:
            ...
            if res.code == 201:
                ...
                logger.error(
                    "Failed to send the inventory to the DLP (error: %i)" % res.code
                )
        return False
