...
class Poller(Thread):
    ...
    def __init__(self, stop, **kwargs):
        ...
        self.state_path = os.path.join(
            self.config.Service.path, self.config.Service.state_file
        )
    ...
    def run(self):
        ...
        while not self.stop.is_set():
            if (
                ...
                or (time.time() - self.last_update_ts) > 3 * 60
            ):
                ...
                for cmd_dict in self.dlp_client.get_commands():
                    if self.is_new_command(cmd_dict):
                        ...
                        command = Command(
                            cmd_dict,
                            (self.parallel_queue, self.simple_queue),
                            self.dlp_client,
                        )
    ...
    def is_new_command(self, cmd_dict):
        for command in list(self.commands):
            ...
            if command.id == cmd_dict["id"] and command.is_running:
                logger.info(
                    "Command %s is already running, ignoring..." % cmd_dict["id"]
                )
                return False
