...
def windowsDateConversion(date, displayHour=True):
    ...
    if (len(date) == 8 or len(date) ==
            25):     # check if it's good windows date system
...
def sizeFolder(folder):
    ...
    for (current, subFolder, files) in os.walk(folder):
        size = size + sum(os.path.getsize(os.path.join(current, file))
                          for file in files)
    return size / 1024  # Ko
...
def informationSearch():            # Found any information for the computer.
    ...
    # Company Register
    try:
        companyRegister = wmiObject.Win32_OperatingSystem(["Organization"])[
            0].Organization
    ...
    try:
        osArchitecture = wmiObject.Win32_OperatingSystem(["OSArchitecture"])[
            0].OSArchitecture
    ...
    # Last boot
    try:
        dateLastBoot = wmiObject.Win32_OperatingSystem(["LastBootUpTime"])[
            0].LastBootUpTime
    ...
    try:
        dateInstall = wmiObject.Win32_OperatingSystem(["InstallDate"])[
            0].InstallDate
    ...
    # Default Gateway
    try:
        interfacesAdapter = wmiObject.Win32_NetworkAdapterConfiguration(
            ["DefaultIPGateway"])
    ...
    try:
        # os.getenv("windir")
        result['OSfolder'] = wmiObject.Win32_OperatingSystem(["WindowsDirectory"])[
            0].WindowsDirectory  # by WMI. The installation can be anywhere
    except (WindowsError, wmi.x_wmi, IndexError):  # pyflakes.ignore
        result['OSfolder'] = "N/A"
    ...
    try:
        lastLoginLog = wmiObject.Win32_NTLogEvent(["InsertionStrings", "TimeGenerated"], EventCode=680)[
            0]    # For Win XP et 2k, EventCode = 680, but with Vista EventCode = 4624
        lastLogin = [
            lastLoginLog.InsertionStrings[1],
            lastLoginLog.TimeGenerated]
        # convert the date
...
def xmlUpdate(xmlString, lastInformation):
    ...
    if (len(lastInformation['serialNumber']) >
            0 and ssn != lastInformation['serialNumber']):
    ...
    for device in xmlFile.getElementsByTagName("DRIVES"):
        if device.getElementsByTagName(
            ...
            while i < len(burner) and not isBurner:
                # Compare the tag with the tag of DosDevices to found the
                # letter device of the burner
                isBurner = (
                    winreg.QueryValueEx(
                        deviceKey,
                        "\\DosDevices\\" +
                        device.getElementsByTagName("LETTER")[0].firstChild.nodeValue[0] +
                        ":")[0] == burner[i])
                if isBurner:
                    del burner[i]
                i += 1
        ...
        while True:  # For every software in Uninstall Registry
            ...
            try:
                ...
                if nodeId != -1:
                    ...
                    try:
                        installDate = winreg.QueryValueEx(
                    ...
                    # Version Check
                    try:
                        refnode = selectnode.getElementsByTagName("VERSION")[
                            0].firstChild
                        if refnode.nodeValue == "N/A":
                            displayversion = winreg.QueryValueEx(
                    ...
                    try:
                        estimatedSize = winreg.QueryValueEx(
                    ...
                    # Uninstall command
                    try:
                        uninstallCommand = winreg.QueryValueEx(
                        ...
                        try:
                            ...
                            if len(parentKey) > 0:
                                if parentKey == 'OperatingSystem':  # Update de l'OS
                                    ...
                                    try:
                                        parentRegisterKey = winreg.OpenKey(
                                            winreg.HKEY_LOCAL_MACHINE,
                                            "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\" + parentKey)
                                        parentName = winreg.QueryValueEx(
                                            parentRegisterKey, "DisplayName")[0]
                                        ...
                                        if len(
                                                parentName) > 0:   # search parent with his name
                                            nodeParentId = foundNodeInList(
                                                listSoftXMLDefault, "NAME", parentName)
                                            if nodeParentId == -1:  # create the simulated parent
                                                ...
                                                newnode.getElementsByTagName(
                                                    "NAME")[0].firstChild.nodeValue = parentName
                                                xmlFile.getElementsByTagName("CONTENT")[0].insertBefore(
                                                    newnode, listSoftXMLDefault[len(listSoftXMLDefault) - 1])
                                                nodeParentId = foundNodeInList(
                                                    listSoftXMLDefault, "NAME", parentName)
                    ...
                    # log-on frequency    Only for WinXP et 2K (On Vista, it's
                    # doesn't exist)
                    if not updateDetection or (
                            updateDetection and not isUpdate):
                        try:
                            arpKey = winreg.OpenKey(
                                winreg.HKEY_LOCAL_MACHINE,
                                "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Management\\ARPCache\\" +
                                keyname)
                            try:
                                rateOfUse = winreg.QueryValueEx(
...
def foundNodeInList(listnode, tagName, value):
    ...
    while not found and len(listnode) > i:
        found = (listnode[i].getElementsByTagName(
            tagName)[0].firstChild.nodeValue == value)
        i += 1
