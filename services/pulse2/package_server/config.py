...
class P2PServerCP(pulse2.utils.Singleton):
    ...
    def packageDetectSmartMethod(self):
        return ', '.join([self.detectSmartMethod[x]
                         for x in self.package_detect_smart_method])
    ...
    def setup(self, config_file):
        ...
        if self.cp.has_option(
                "main", "bind"):  # TODO remove in a future version
        ...
        for section in self.cp.sections():
            ...
            if re.compile('^package_api_get:[0-9]+$').match(section):
                ...
                self.package_api_get.append(
                    {'mount_point': mount_point, 'src': src})
            if re.compile('^package_api_put:[0-9]+$').match(section):
                ...
                self.package_api_put.append(
                    {'mount_point': mount_point, 'src': src, 'tmp_input_dir': pap_tmp_input_dir})
        ...
        if self.cp.has_section("imaging_api") and self.cp.has_option(
            ...
            if self.cp.has_option('imaging_api', 'bootloader_folder'):
                bootloader_folder = self.cp.get(
                    'imaging_api', 'bootloader_folder')
            if self.cp.has_option('imaging_api', 'cdrom_bootloader'):
                cdrom_bootloader = self.cp.get(
                    'imaging_api', 'cdrom_bootloader')
            if self.cp.has_option('imaging_api', 'bootsplash_file'):
                bootsplash_file = self.cp.get(
                    'imaging_api', 'bootlsplash_file')
            if self.cp.has_option('imaging_api', 'bootmenus_folder'):
                bootmenus_folder = self.cp.get(
                    'imaging_api', 'bootmenus_folder')
            ...
            if self.cp.has_option('imaging_api', 'diskless_initrdcd'):
                diskless_initrdcd = self.cp.get(
                    "imaging_api", 'diskless_initrdcd')
            if self.cp.has_option('imaging_api', 'computers_folder'):
                computers_folder = self.cp.get(
                    'imaging_api', 'computers_folder')
            if self.cp.has_option('imaging_api', 'inventories_folder'):
                inventories_folder = self.cp.get(
                    'imaging_api', 'inventories_folder')
            ...
            if self.cp.has_option('imaging_api', 'rpc_replay_file'):
                rpc_replay_file = os.path.join(
                    base_folder, self.cp.get(
                        'imaging_api', 'rpc_replay_file'))
            if self.cp.has_option('imaging_api', 'rpc_loop_timer'):
                rpc_loop_timer = self.cp.getint(
                    'imaging_api', 'rpc_loop_timer')
            ...
            if self.cp.has_option("imaging_api", 'inventory_enablessl'):
                inventory_enablessl = self.cp.get(
                    "imaging_api", 'inventory_enablessl')
            if not isUUID(uuid):
                raise TypeError(
        ...
        if self.cp.has_option("main", "package_mirror_target"):
            ...
            if (
                isinstance(
                    self.package_mirror_target,
                    str) and self.package_mirror_target != '') or (
                isinstance(
                    self.package_mirror_target,
                    list) and len(
                    self.package_mirror_target) == 1 and self.package_mirror_target[0] != '') or (
                        isinstance(
                            self.package_mirror_target,
                            list) and len(
                                self.package_mirror_target) != 1):
            ...
            if self.cp.has_option("main", "package_global_mirror_activate"):
                ...
                if self.cp.has_option("main",
                    ...
                    self.package_global_mirror_command_options = self.cp.get(
                        "main", "package_global_mirror_command_options").split(' ')
...
def add_access(mirror_params, conf):
    ...
    mirror_params['file_access_path'] = "%s_files" % (
