...
class Description(resource.Resource):
    ...
    def render_GET(self, request):
        ...
        for description in self.services:
            style = ""
            if (
                "server" in request.args
                and request.args["server"] == description["server"]
                and "port" in request.args
                and int(request.args["port"]) == int(description["port"])
                and "proto" in request.args
                and request.args["proto"] == description["proto"]
                and "mp" in request.args
                and request.args["mp"] == description["mp"]
                and "type" in request.args
                and request.args["type"] == description["type"]
            ):
                style = "selected"
