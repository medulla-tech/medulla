...
class PluginInventoryUPAAConfig(Singleton):
    dbdriver = "mysql"
    dbhost = "localhost"
    dbname = "inventory"
    dbuser = "mmc"
    ...
    def setup(self, config_file):
        ...
        for section in self.cp.sections():
            if re.compile('^associations:[0-9]+$').match(section):
                if not self.cp.has_option(section, 'entity'):
                    continue
                if not self.cp.has_option(section, 'package_api_put'):
                    continue
                if not self.cp.has_option(section, 'kind'):
                    continue
                
                entities = self.cp.get(section, 'entity').split('||')
                paps = self.cp.get(section, 'package_api_put').split('||')
                kind = self.cp.get(section, 'kind')
                for entity in entities:
                    if not self.entity2pap.has_key(entity):
                        self.entity2pap[entity] = {}
                    if not self.entity2pap[entity].has_key(kind):
                        self.entity2pap[entity][kind] = []
                    self.entity2pap[entity][kind].extend(paps)

                for pap in paps:
                    if not self.pap2entity.has_key(pap):
                        self.pap2entity[pap] = {}
                    if not self.pap2entity[pap].has_key(kind):
                        self.pap2entity[pap][kind] = []
                    self.pap2entity[pap][kind].extend(entities)
                    
        if len(self.pap2entity.keys()) == 0:
