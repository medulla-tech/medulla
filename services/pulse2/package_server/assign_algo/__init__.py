...
class IntAssignAlgoManager(pulse2.utils.Singleton):
    ...
    def getAlgo(self, assign_algo):
        ...
        logger.debug("Using the %s %s Assign Algorythm"%(assign_algo, self.name))
        if wanted != assign_algo:
            logger.warning("Can't load the wanted one (conf:%s, use:%s)"%(wanted, assign_algo))
        return algo
    ...
    def _getAlgo(self, assign_algo):
        try:
            ...
            logger.debug("Algo search path: %s" % searchpath)
            ...
            if assign_algo != 'default':
                ...
                logger.error("Cant load any %s Assign Algorythm"%(self.name))
                ret = None
