...
class Command:
    def __init__(self, self, name = '', command = ''):
        self.name = name
...
class Package:
    ...
    def equal(self, p):
        if self.label != p.label or self.version != p.version or self.size != p.size or self.id != p.id or
                not(self.initcmd.equal(p.initcmd)) or not(self.precmd.equal(p.precmd)) or not(self.cmd.equal(p.cmd)) or
                not(self.postcmd_ok.equal(p.postcmd_ok)) or not(self.postcmd_ko.equal(p.postcmd_ko)) or
...
class File:
    ...
    def toURI(self):
        return "%s://%s:%s%s%s/%s" % (proto, server_uri, str(server_port), server_mp, path, name)
