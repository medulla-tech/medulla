...
class ISOImage:
    ...
    def __init__(self, config, imageUUID, size, title):
        ...
        if not os.path.isdir(targetdir):
            raise Exception(
                "Target directory %s for ISO image doesn't exist" %
                targetdir)
    ...
    def _makeGRUBMenu(self, medialist):
        ...
        try:
            ...
            fid.close()
            self.logger.info(
                "boot CD menu %s successfuly created" %
                (menufile))
    ...
    def _finishMediaList(self, medialist, medianumber):
        ...
        logging.getLogger('imaging').info(
            "Iso image : starting volumes generation, max size is %s" %
            (self.size))
    ...
    def create(self):
        ...
        def _cbSuccess(result):
            ...
            if code:
                self.logger.error(
                    'Error while generating ISO image: error code = %d' %
                    code)
        ...
        def _cbError(result):
            ...
            self.logger.error(
                'Signal %d received while generating ISO image' %
                signalNum)
            return False
