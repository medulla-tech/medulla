...
class MscDatabase(DatabaseHelper):
    ...
    def get_msc_listuuid_commandid(
            self,
            session,
            command_id,
            filter,
                    ...
                    CommandsOnHostPhase.fk_commands_on_host == CommandsOnHost.id) .filter(
                        CommandsOnHostPhase.name == 'upload') .filter(
                            CommandsOnHostPhase.state == 'ready') .filter(
                                Commands.end_date > datereduced) .filter(
                                    Commands.type != 2)
...
#        ret = ret.filter(self.commands_on_host.c.id == self.target.c.fk_commands_on_host)
    ...
    def getAllCommandsConsult(self, ctx, min, max, filt, expired=True):
        ...
        query = query.group_by(
            func.ifnull(
                self.commands.c.fk_bundle, func.md5(
                    self.commands.c.id)))  # .group_by(self.commands.c.id)
...
#   msc.displayLogs({'max': 10, 'finished': '', 'filt': '', 'uuid': 'UUID1620', 'min': 0},)
    ...
    def getLastCommandsOncmd_id_start_end(self, ctx, cmd_id):
        ...
        ret = session.query(
            ...
            Commands.package_id) .join(Commands) .filter(
            self.commands_on_host.c.fk_commands == cmd_id).order_by(
            desc(
                self.commands_on_host.c.id)).first()
        session.close()
    ...
    def antiPoolOverflowErrorback(self, reason):
        ...
        if self.db.pool._max_overflow > - \
                1 and self.db.pool._overflow >= self.db.pool._max_overflow:
