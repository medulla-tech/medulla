...
class MasteringDatabase(DatabaseHelper):
    ...
    @DatabaseHelper._sessionm
    def get_actions_for_entity(self, session, server, entity=-1, _type="all", uuid="", gid="", start=0, end=-1, _filter=""):
        """
...
WHERE """
        ...
        # Search based on server entity id
            ...
            binds["entity_id1"] = entity
                ...
                # Add filter on type of action (known machine, new machine, group, all entity)
        if _type == "machine":
            sql += """AND (uuid != "" and uuid is not null) """
        elif _type == "group":
            sql += """AND (gid != "" and gid is not null) """
        elif _type == "new":
            sql += """AND (uuid = "" or uuid is null) and (gid = "" or gid is null) """
