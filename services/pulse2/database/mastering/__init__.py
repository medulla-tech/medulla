...
"""
    ...
    def get_mastering_scripts_list(self, session, server, entity, start, end, _filter):
        ...
        Args:
            self (MasteringDatabase) : instance of class
            session (sqlalchemy.session) : session to access to the database. Generated by decorator
            server: the server jid to get scripts for. This is used to get scripts for a specific server.
            entity: the entity id to get scripts for. This is used to get scripts for a specific entity.
            start: pagination parameter to specify the offset of the first element.
            end: pagination parameter to specify the last element. -1 to get all elements.
            _filter: filter to apply on results. This is used to filter results based on their content.

        Return:
            A dict with total number of mastering scripts and the list of mastering scripts.
            The dict have the following shape:
            {
                "total": 0,
                "data": [
                    {
                        ...
                        "modification_date": master modification date
                    },
                    ...
                ]
            }"""

        # trivial case : no entity, no script
        if entity == "":
            return {
                "total": 0,
                "data": []
...
WHERE entity_id = :entity """
        ...
        if _filter != "":
            sql += """AND (name like :filt1 OR description like :filt2 OR creation_date like :filt3 or modification_date like :filt4) """
            binds["filt1"] = f"%{_filter}%"
            binds["filt2"] = f"%{_filter}%"
            binds["filt3"] = f"%{_filter}%"
            binds["filt4"] = f"%{_filter}%"

        sql += """ORDER BY name, creation_date ASC """
        sql += limit_str
    ...
    def add_mastering_script(self, session, server, entity, name, description, content):
        ...
        try:
            ...
            session.rollback()
            logger.error("Failed to add mastering script : %s <=>%s"%(name, e))
    ...
    @DatabaseHelper._sessionm
    def get_summary_scripts_list(self, session, entity):
        """Get the list of mastering scripts names and description for a specific entity. This is used to get an overview of available scripts for an entity.

        Args:
            self (MasteringDatabase) : instance of class
            session (sqlalchemy.session) : session to access to the database. Generated by decorator
            entity (int) : the entity id to get scripts for.

        Return:
            A list of dict with script name and description.
            [{"id":script_id, "name":script_name, "description":script_description} ...]"""

        sql = """SELECT id, name, description FROM scripts WHERE entity_id = :entity_id ORDER BY name ASC"""
        binds = {
            "entity_id": entity,
        }
