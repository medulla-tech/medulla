...
from pulse2.dlp.tests.utils import (
    run_agent,
    run_tests,
    stop_agent,
    HTTPClient,
...
class TestAuth(unittest.TestCase):
    ...
    def testBasHostname(self):
        with self.assertRaises(HTTPError) as context:
            self.client.post(
                "/auth",
                {"authkey": "TEST", "hostname": "FOO", "mac_list": "AA:BB:CC:DD:EE:FF"},
            )
    ...
    def testCorrectAuth(self):
        result = self.client.post(
            "/auth",
            {"authkey": "TEST", "hostname": "test1", "mac_list": "AA:BB:CC:DD:EE:FF"},
        )
    ...
    def testMultipleMACs(self):
        result = self.client.post(
            "/auth",
            {
                "authkey": "TEST",
                "hostname": "test1",
                "mac_list": ["AA:BB:CC:DD:EE:FF", "AA:BB:CC:DD:EE:00"],
            },
        )
        self.assertEqual(result.code, 200)
    ...
    def testBadMAC(self):
        with self.assertRaises(HTTPError) as context:
            self.client.post(
                "/auth",
                {"authkey": "TEST", "hostname": "test1", "mac_list": "AA:BB:CC"},
            )
    ...
    def testBadParams(self):
        with self.assertRaises(HTTPError) as context:
            self.client.post(
                "/auth", {"authkey": "TEST", "host": "test1", "macs": "AA:BB:CC"}
            )
        self.assertEqual(context.exception.code, 404)
...
class TestCommands(unittest.TestCase):
    ...
    def testGetCommands(self):
        ...
        self.assertEqual(result.code, 200)
        self.assertEqual(
            result.data[0]["package_uuid"], "d13d3eaa-587a-11e3-adfa-080027fd96ca"
        )
...
class TestFile(unittest.TestCase):
    ...
    def testNoRightsDownload(self):
        ...
        self.client.post(
            "/auth",
            {"authkey": "TEST", "hostname": "test2", "mac_list": "AA:BB:CC:DD:EE:FF"},
        )
        self.client.get("/commands")
...
class TestStep(unittest.TestCase):
    ...
    def testSendResult(self):
        ...
        result = self.client.post(
            "/step/1/upload/",
            data={"stdout": "stdout", "stderr": "stderr", "return_code": 0},
        )
