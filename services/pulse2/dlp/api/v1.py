...
class Auth(object):
    exposed = True
    _cp_config = {"tools.json_out.on": True, "tools.xmlrpc_client.on": True}
    ...
    def POST(self, authkey, mac_list, hostname):
        ...
        if not authkey == cherrypy.config.get("dlp.authkey"):
            log(
                "Failed to authenticate computer %s, authkey missmatch." % hostname,
        ...
        try:
            ...
            if uuid is not None and uuid:
                ...
                log(
                    "Not recognized machine, hostname=%s, mac_list=%s"
                    % (hostname, mac_list),
...
class Commands(object):
    ...
    def check_cache(self, package_uuid, package_path):
        if os.path.exists(package_path):
            ...
            if (
                current_time - cherrypy.config.get("dlp.cache_expire")
                > stat_info.st_ctime
            ):
                return False
            else:
    ...
    def create_package(self, package_uuid, package_path, command):
        ...
        if not os.path.exists(lock_file):
            try:
                ...
                log(
                    "Failed to create the package %s" % package_uuid,
                    logging.ERROR,
                    True,
                )
                result = False
                ...
                if package_uuid:
                    ...
                    if self.check_cache(package_uuid, package_path):
                        ...
                        if not self.create_package(package_uuid, package_path, command):
                            del commands[index]
                            log(
                                "Command %d removed from list" % command["id"],
                                logging.ERROR,
                            )
                            continue
...
class File(object):
    ...
    def GET(self, filename):
        ...
        allowed_packages = [
            "%s.zip" % c["package_uuid"] for c in commands if "package_uuid" in c
        ]
...
class Step(object):
    ...
    def POST(self, coh_id, step_id, stdout, stderr, return_code):
        ...
        try:
            log(
                "Saving result (%d) for step %s of command %s"
...
class Inventory(object):
    ...
    def POST(self, inventory):
        ...
        try:
            ...
            log(
                "Failed to send the inventory to the inventory server",
                logging.ERROR,
                True,
            )
            raise cherrypy.HTTPError(503, "Service Unavailable")
