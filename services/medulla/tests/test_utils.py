...
# This file make unit tests for the content of medulla/utils.py

"""
Tests for medulla.utils
"""
...
# Imports for unitest
import unittest
from medulla.utils import isMACAddress, isUUID, splitComputerPath
...
def test_singleton():
    """Tests the Singleton object in medulla/utils.py
    Test 1"""

    class UniqueSingleton(medulla.utils.Singleton):
...
def test_singleton_n():
    """Tests the SingletonN object in medulla/utils.py
    Test 2"""

    class UniqueSingletonN(object, metaclass=medulla.utils.SingletonN):
...
class TestMedulla2ConfigParser(object):
    """Tests the Medulla2ConfigParser object"""

    a = medulla.utils.Medulla2ConfigParser()
...
class TestXmlrpcCleanup(object):
    ...
    def test_with_date(self):
        ...
        assert medulla.utils.xmlrpcCleanup(the_date) == tuple(the_date.timetuple())
    ...
    def test_with_string(self):
        ...
        Test 10"""
        assert medulla.utils.xmlrpcCleanup("default") == "default"

    def test_with_int(self):
        """Test xmlrpcCleanup with int
        Test 11"""
        assert medulla.utils.xmlrpcCleanup(3) == 3

    def test_with_none_value(self):
        """Test xmlrpcCleanup with None value
        Test 12"""
        assert medulla.utils.xmlrpcCleanup(None) == False

    def test_with_object(self):
        """Test xmlrpcCleanup with custom object
        Test 13"""
        my_object = self.MyClass()
        assert medulla.utils.xmlrpcCleanup(my_object) == my_object

    def test_with_long(self):
        """Test xmlrpcCleanup with long
        Test 14"""
        assert medulla.utils.xmlrpcCleanup(int(3)) == "3"
    ...
    def test_with_tuple(self):
        ...
        Test 15"""
        the_date = datetime(2018, 0o2, 3)
        assert medulla.utils.xmlrpcCleanup((1, int(2), "3", the_date, None)) == [
            1,
            "2",
            "3",
    ...
    def test_with_dict(self):
        ...
        assert medulla.utils.xmlrpcCleanup(the_dict) == {
            "a": 4,
            "1": "value",
            "key": False,
        }
...
class TestUnique(object):
    ...
    def test_with_string(self):
        """Test with string
        Test 20"""
        assert medulla.utils.unique("test") == ["s", "e", "t"]

    def test_with_int(self):
        """Test with int
        Test 21"""
        with pytest.raises(TypeError):
            medulla.utils.unique(2)
...
class TestSameNetwork(object):
    ...
    def test_with_wrong_string(self):
        ...
        ip2 = "wrong ip2"
        mask = "wrong mask"
        assert medulla.utils.same_network(ip1, ip2, mask) == False
    ...
    def test_with_right_string_same_network(self):
        ...
        ip2 = "192.168.1.140"
        mask = "255.255.255.0"
        assert medulla.utils.same_network(ip1, ip2, mask)
    ...
    def test_with_int(self):
        ...
        with pytest.raises(AttributeError):
            # Generate an error
            medulla.utils.same_network(ip1, ip2, mask)
...
class TestOnlyAddNew(object):
    ...
    def test_with_none(self):
        ...
        value = None
        with pytest.raises(AttributeError):
            medulla.utils.onlyAddNew(obj, value)
    ...
    def test_unique_value_not_in_obj(self):
        ...
        obj = []
        value = 4
        assert medulla.utils.onlyAddNew(obj, value) == [4]
    ...
    def test_list_not_in_obj(self):
        ...
        assert medulla.utils.onlyAddNew(obj, value) == [4, 1, 2, 3, 5, 6, 7]
...
class TestGetConfigFile(object):
    ...
    def test_with_string_and_default_path(self):
        ...
        Test 38"""
        assert (
            medulla.utils.getConfigFile("my_module") == "/etc/mmc/plugins/my_module.ini"
        )
    ...
    def test_with_string_and_string_path(self):
        ...
        Test 39"""
        assert (
            medulla.utils.getConfigFile("my_module", "my/new/path/")
    ...
    def test_with_string_and_int_path(self):
        ...
        Test 40"""
        with pytest.raises(AttributeError):
            medulla.utils.getConfigFile("my_module", 4)

    def test_with_string_and_none_path(self):
        """Test the function with another path
        Test 41"""
        with pytest.raises(AttributeError):
            medulla.utils.getConfigFile("my_module", None)

    def test_with_string_and_empty_string_in_path(self):
        """Test the function with another path
        Test 42"""
        assert medulla.utils.getConfigFile("my_module", "") == "my_module.ini"
...
class TestGrepv(object):
    ...
    def test_with_unexpected_parameters(self):
        ...
        Test58"""
        with pytest.raises(TypeError):
            medulla.utils.grepv(1, 3)
            medulla.utils.grepv(1, [1, 2, 3])
            medulla.utils.grepv("Test", [1, 2, 3])

    def test_with_expected_parameters(self):
        """Tests the function without parameters
        Test59"""
        assert medulla.utils.grepv("1", ["1", "2", "3"]) == ["2", "3"]

        # Test 60
        assert medulla.utils.grepv("a", "arbre") == ["r", "b", "r", "e"]
