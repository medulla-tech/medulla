...
from medulla.database.inventory import InventoryCreator, Inventory
from medulla.database.inventory.mapping import OcsMapping
from medulla.database.inventory.entitiesrules import EntitiesRules, DefaultEntityRules
from medulla.utils import Singleton
from mmc.site import mmcconfdir
from medulla.inventoryserver.config import Medulla2OcsserverConfigParser
from medulla.inventoryserver.ssl import (
    SecureHTTPRequestHandler,
    SecureThreadedHTTPServer,
)
from medulla.inventoryserver.utils import InventoryUtils, canDoInventory
from medulla.inventoryserver.scheduler import AttemptToScheduler
from medulla.inventoryserver.glpiproxy import (
    GlpiProxy,
...
class HttpInventoryServer(http.server.BaseHTTPRequestHandler, InventoryServer):
    def __init__(self, *args):
        ...
        self.config = Medulla2OcsserverConfigParser()
...
class TreatInv(Thread):
    ...
    def treatinv(self, deviceid, from_ip, cont):
        ...
        try:
            ...
            try:
                path = Medulla2OcsserverConfigParser().hostname
            ...
            if isinstance(result, list) and len(result) == 2:
                # disabling light pull on GLPI mode when Medulla2 inventory
                # creator is enabled
                if (
