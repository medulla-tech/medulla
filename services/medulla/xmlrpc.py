...
class OpenSSLContext(medulla.utils.Singleton):
    """
    Singleton Class to hold OpenSSL stuff, preventing further reopening of client certs

    use medulla.xmlrpc.OpenSSLContext().setup(localcert_path, cacert_path, verifypeer) to setup (first time usage)
    use medulla.xmlrpc.OpenSSLContext().getContext() to get ctx
...
class Medulla2XMLRPCProxy(Proxy):
    def __init__(
        self,
        url,
        user=None,
