...
if Medulla2InventoryProxyConfig().addicon:
    import win32ui
    import win32gui
    import win32con
    import win32api
...
def windowsDateConversion(date, displayHour=True):
    if len(date) == 8 or len(date) == 25:  # check if it's good windows date system
        ...
        if displayHour:
            finaldate = (
                finaldate + " " + date[8:10] + ":" + date[10:12] + ":" + date[12:14]
            )
...
def informationSearch():  # Found any information for the computer.
    ...
    try:
        ...
        result["OSfolder"] = wmiObject.Win32_OperatingSystem(["WindowsDirectory"])[
            0
        ].WindowsDirectory  # by WMI. The installation can be anywhere
    except (WindowsError, wmi.x_wmi, IndexError):  # pyflakes.ignore
        result["OSfolder"] = "N/A"
    ...
    try:
        lastLoginLog = wmiObject.Win32_NTLogEvent(
            ["InsertionStrings", "TimeGenerated"], EventCode=680
        )[
            0
        ]  # For Win XP et 2k, EventCode = 680, but with Vista EventCode = 4624
...
def xmlUpdate(xmlString, lastInformation):
    ...
    if (
        len(lastInformation["serialNumber"]) > 0
    ...
    for device in xmlFile.getElementsByTagName("DRIVES"):
        if (
            device.getElementsByTagName("TYPE")[0].firstChild.nodeValue
            == "CD-Rom Drive"
        ):
            isBurner = False
            ...
            while i < len(burner) and not isBurner:
                ...
                isBurner = (
                    winreg.QueryValueEx(
                        deviceKey,
                        "\\DosDevices\\"
                        + device.getElementsByTagName("LETTER")[0].firstChild.nodeValue[
                            0
                        ]
                        + ":",
                    )[0]
    ...
    updateDetection = Medulla2InventoryProxyConfig().updatedetection
        ...
        while True:  # For every software in Uninstall Registry
            ...
            try:
                ...
                if nodeId != -1:
                    ...
                    selectnode.getElementsByTagName("FILESIZE")[
                        0
                    ].firstChild.nodeValue = estimatedSize
                            ...
                            if len(parentKey) > 0:
                                if parentKey == "OperatingSystem":  # Update de l'OS
                                    ...
                                    try:
                                        ...
                                        if (
                                            len(parentName) > 0
                                        ):  # search parent with his name
                                            nodeParentId = foundNodeInList(
                                                listSoftXMLDefault, "NAME", parentName
                                            )
                                            if (
                                                nodeParentId == -1
                                            ):  # create the simulated parent
                                                ...
                                                for j in range(0, len(nodeslist)):
                                                    if j % 2 == 1:
                                                        nodeslist[
                                                            j
                                                        ].firstChild.nodeValue = "N/A"

                                                newnode.getElementsByTagName("NAME")[
                                                    0
                                                ].firstChild.nodeValue = parentName
                                                xmlFile.getElementsByTagName("CONTENT")[
                                                    0
                                                ].insertBefore(
                                                    newnode,
                                                    listSoftXMLDefault[
                                                        len(listSoftXMLDefault) - 1
                                                    ],
                                                )
                                                nodeParentId = foundNodeInList(
                                                    listSoftXMLDefault,
                                ...
                                if nodeParentId > -1:
                                    ...
                                    selectnode.setAttribute("parent", parentName)
                        ...
                        if len(refnode) == 0:
                            ...
                            if valueRateOfUse != "N/A":
                                if refnode[0].firstChild.valueNode != "N/A":
                                    refnode[0].firstChild.valueNode = (
...
def foundNodeInList(listnode, tagName, value):
    ...
    while not found and len(listnode) > i:
        found = (
            listnode[i].getElementsByTagName(tagName)[0].firstChild.nodeValue == value
        )
        i += 1
...
def improveXML(xmlFileString):
    ...
    if Medulla2InventoryProxyConfig().savexmlmodified:
...
def getOcsDebugLog(xmlString):
    ...
    ocsFolder = Medulla2InventoryProxyConfig().command_name
    i = len(ocsFolder)
    found = False
