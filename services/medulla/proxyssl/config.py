...
from medulla.proxyssl.utilities import Singleton
import configparser
import sys


class Medulla2InventoryProxyConfig(Singleton):
    server = ""
    port = 9999
    local_port = 9999
    path = "/"
    ...
    def setup(self, config_file="conf/p2ipc.ini"):
        ...
        if self.cp.has_option("polling", "activate") and self.cp.getboolean(
            "polling", "activate"
        ):
            self.polling = True
            if (
                self.cp.has_option("polling", "type")
                and self.cp.get("polling", "type") == "reg"
            ):
                self.flag_type = "reg"
            else:
