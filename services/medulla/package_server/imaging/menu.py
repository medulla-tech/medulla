...
import medulla.package_server.imaging.api.functions
...
class ImagingMenu:
    ...
    def __init__(self, config, macaddress=None, hostname=None, uuid=None):
        ...
        if macaddress:
            assert medulla.utils.isMACAddress(macaddress)
    ...
    def _applyReplacement(self, string, condition="global"):
        ...
        if self.mac:
            replacements.append(
                ("##MAC##", medulla.utils.reduceMACAddress(self.mac), "global")
            )
    ...
    def buildMenu(self):
        ...
        # takes global items, one by one
        buf = "# Auto-generated by Medulla 2 Imaging Server on %s \n\n" % time.asctime()
        buf += "UI vesamenu.c32\n"
        buf += "PROMPT 0\n"
    ...
    def write(self):
        ...
        if self.mac:
            filename = os.path.join(
                ...
                medulla.utils.normalizeMACAddressForPXELINUX(self.mac),
            )
            self.logger.debug(
        ...
        # Retrieve PXE Params
        medulla.package_server.imaging.api.functions.Imaging().refreshPXEParams(
            _write, self
        )
    ...
    def clear(self):
        for i in self.mac:
            ...
            filename = os.path.join(
                self.config.imaging_api["base_folder"],
                self.config.imaging_api["bootmenus_folder"],
                medulla.utils.normalizeMACAddressForPXELINUX(i),
            )
            try:
                os.unlink(filename)
            except Exception as e:
...
def changeDefaultMenuItem(macaddress, value):
    ...
    filename = os.path.join(
        ...
        medulla.utils.normalizeMACAddressForPXELINUX(macaddress),
    )
...
class ImagingMulticastMenuBuilder:
    ...
    def __init__(self, menu):
        ...
        # if medulla.utils.get_ip_address(interface)==self.public_ip:
...
Replacement of historic Medulla PXE by PXELINUX
MENU WIDTH 78
MENU MARGIN 4
MENU ROWS 10
MENU VSHIFT 10
...
INITRD ../davos/initrd.img"""
    ...
    def chooseMacAddress(self):
        ...
        for k, v in self.menu["computer"].iteritems():
            mac = medulla.utils.reduceMACAddress(k)
            filename = medulla.utils.normalizeMACAddressForPXELINUX(mac)
