...
"""
Classes and methods related to the build of Medulla 2 ISO images.
"""
...
from medulla.imaging.image import (
...
class ISOImage:
    ...
    def __init__(self, config, imageUUID, size, title):
        """
        @param imageUUID: UUID of the Medulla 2 image to convert to an ISO
        @type imageUUID: str
        ...
        if not isMedulla2Image(self.source):
        ...
        if not os.path.isdir(targetdir):
            raise Exception(
                "Target directory %s for ISO image doesn't exist" % targetdir
            )
        ...
        self.tempdir = tempfile.mkdtemp("medulla-iso")
        self.medialist = {}
    ...
    def _makeGRUBMenu(self, medialist):
        ...
        buf.append("desc %s\n" % "FIXME")
        buf.append(
            "kernel (cd)/bzImage revorestorenfs revosavedir=/cdrom quiet revonospc\n"
        )
        ...
        try:
            ...
            self.logger.error(
                "failed to create boot CD menu %s (c=%s: %s)"
                % (menufile, errno, strerror)
            )
            raise
    ...
    def _finishMediaList(self, medialist, medianumber):
        ...
        logging.getLogger("imaging").info(
            "Iso image : starting volumes generation, max size is %s" % (self.size)
        )

        grubfile = os.path.join(
            self.config.imaging_api["base_folder"],
        ...
        medialist[medianumber]["files"][len(medialist[medianumber]["files"])] = {
            "src": grubfile,
            "dst": "grub.cdrom",
        }

        bzimagefile = os.path.join(
            self.config.imaging_api["base_folder"],
            self.config.imaging_api["diskless_folder"],
        ...
        for name in os.listdir(self.source):
            ...
            if os.path.isfile(filepath) and name not in priofiles:
                ...
                if medialist[medianumber]["mediasize"] + filesize > self.size:
                    medianumber += 1
                    logging.getLogger("imaging").info(
                        "Iso image : max size of %s reached, stopping at %s and switching to volume %s"
                        % (
                            self.size,
                            medialist[medianumber - 1]["mediasize"],
                            medianumber,
                        )
                    )
                    medialist[medianumber] = {"mediasize": 0, "files": {}, "opts": ""}
                medialist[medianumber]["mediasize"] += filesize
                medialist[medianumber]["files"][
                    len(medialist[medianumber]["files"])
                ] = {"src": filepath, "dst": name}
    ...
    def create(self):
        ...
        def _cbSuccess(result):
            ...
            if code:
                self.logger.error(
                    "Error while generating ISO image: error code = %d" % code
                )
        ...
        def _cbError(result):
            ...
            self.logger.error(
                "Signal %d received while generating ISO image" % signalNum
            )
            return False
        ...
        for medianumber in self.medialist:
            ...
            args += ["-V", "FIXME TITLE"]
            # Application ID
            args += ["-A", "Medulla 2 Imaging Server"]
            # Output file
            ...
            d = getProcessOutputAndValue(self.config.imaging_api["isogen_tool"], args)
            d.addCallbacks(_cbSuccess, _cbError)
            self.logger.debug(
                "Creating ISO image %s-%d.iso from image %s"
                % (self.target, medianumber + 1, self.imageUUID)
            )
