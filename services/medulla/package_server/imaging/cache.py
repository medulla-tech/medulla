...
import medulla.package_server.config

# FIXME : shouldn't this cache be updated by the object itself ?


class UUIDCache(medulla.utils.Singleton):
    """
    ...
    def __init__(self):
        medulla.utils.Singleton.__init__(self)
        self.cachePath = medulla.package_server.config.P2PServerCP().imaging_api[
            "uuid_cache_file"
        ]
        self.cacheLifetime = medulla.package_server.config.P2PServerCP().imaging_api[
            "uuid_cache_lifetime"
        ]
    ...
    def getByMac(self, mac):
        ...
        if not medulla.utils.isMACAddress(mac):
            return False

        mac = medulla.utils.normalizeMACAddress(mac)
    ...
    def getByUUID(self, uuid):
        ...
        if not medulla.utils.isUUID(uuid):
            return False
