...
from medulla.package_server.config import config_addons
from medulla.package_server.common import Common
from medulla.package_server.common.serializer import PkgsRsyncStateSerializer
    ...
    def _runSub(self):
        ...
        for pid, targets, pkg in Common().getPackagesThatNeedRsync():
            ...
            if not os.path.exists(p_dir):
                # deletion = mirror empty dir + mirror top level on just 1
                # level
                os.mkdir(p_dir)
                # mark as deletion
                is_deletion = True
...
class ThreadLauncher(medulla.utils.Singleton):
    def initialize(self, config):
        ...
        from medulla.package_server import thread_webserver
