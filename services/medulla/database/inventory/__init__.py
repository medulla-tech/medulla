...
from medulla.database.inventory.mapping import OcsMapping
from medulla.utils import same_network, Singleton, isUUID
...
class Inventory(DyngroupDatabaseHelper):
    """
    Class to query the LRS/Medulla2 inventory database, populated by OCS inventory.
    ...
    def initMappers(self):
        ...
        for item in self.config.getInventoryParts():
            ...
            has_columns = [
                ...
                Column(
                    "inventory", Integer, ForeignKey("Inventory.id"), primary_key=True
                ),
                Column(
                    item.lower(), Integer, ForeignKey(item + ".id"), primary_key=True
                ),
            ]
            if item in noms:
    ...
    def __machinesOnlyQuery(self, ctx, pattern=None, session=None, count=False):
        ...
        join_query = (
            join_query.join(self.table["hasEntity"])
            .join(self.table["Entity"])
            .join(self.inventory)
        )
            ...
            if {"os", "user", "type", "domain", "fullname"} & requested_cols:
                join_query = join_query.outerjoin(
                    ...
                    and_(
                        ...
                        self.table["hasHardware"].c.inventory == self.inventory.c.id,
                    ),
                ).outerjoin(
                    self.table["Hardware"],
            ...
            if "Registry" in self.config.content:
                join_query = join_query.outerjoin(
                    ...
                    and_(
                        ...
                        self.table["hasRegistry"].c.inventory == self.inventory.c.id,
                    ),
                ).outerjoin(
                    self.table["Registry"],
        ...
        if pattern:
            ...
            if "hostname" in pattern and pattern["hostname"].strip():
                ...
                clauses.append(
                    self.machine.c.Name.like("%" + pattern["hostname"] + "%")
                )
                ...
                if {"macAddress", "ipHostNumber", "subnetMask"} & requested_cols:
                    ...
                    clauses.append(
                        self.table["Network"].c.IP.like("%" + pattern["hostname"] + "%")
                    )
                    clauses.append(
            ...
            if "filter" in pattern:
                query = query.filter(
                    self.machine.c.Name.like("%" + pattern["filter"] + "%")
                )
            if "uuid" in pattern:
            ...
            if "uuids" in pattern and isinstance(pattern["uuids"], list):
                if len(pattern["uuids"]) > 0:
                    query = query.filter(
                        self.machine.c.id.in_([fromUUID(m) for m in pattern["uuids"]])
                    )
                else:
                    query = query.filter("1 = 0")
            if "location" in pattern and pattern["location"]:
                query = query.filter(
                    self.table["Entity"].c.id == fromUUID(pattern["location"])
                )
        ...
        if "max" in pattern:
            if pattern["max"] != -1:
                if (
                    (
                        "gid" in pattern
                        and ComputerGroupManager().isrequest_group(ctx, pattern["gid"])
                    )
                    or "gid" not in pattern
                    or "gid" in pattern
                    and "location" in pattern
                ):
    ...
    def optimizedQuery(self, ctx, filt):
        ...
        result = self.getLastMachineInventoryPart(
            ctx, "Registry", {"where": [criterion, values]}
        )
    ...
    def getComputersOptimized(self, ctx, filt):
        ...
        if len(tables) == 1 and "Registry" in tables:
            ...
            for machine in result:
                ...
                tmp = [False, {"cn": [machine.Name], "objectUUID": [uuid]}]
                computers[uuid] = tmp
                machine_attributes = machine.toDN(ctx)[1]
                for k, v in list(machine_attributes.items()):
    ...
    def mapping(self, ctx, query, invert=False):
        ...
        # search
            ...
            if value.startswith(">") and not invert or value.startswith("<") and invert:
                ...
                return and_(
                    getattr(partKlass, field) > value, self.inventory.c.Last == 1
                )
            elif (
                value.startswith(">") and invert or value.startswith("<") and not invert
            ):
                value = value.replace(">", "").replace("<", "")
                return and_(
                    getattr(partKlass, field) < value, self.inventory.c.Last == 1
                ...
                value = value.replace("*", "%")
                return and_(
                    getattr(partKlass, field).like(value), self.inventory.c.Last == 1
                )
        # elif query[2] in PossibleQueries().possibleQueries('halfstatic'): #
        # halfstatic search
        elif query[2] in PossibleQueries().possibleQueries(
            "halfstatic"
        ):  # halfstatic search
            if table == "Machine":
                partKlass = Machine
            ...
            if value.startswith(">") and not invert or value.startswith("<") and invert:
                ...
                value = value.replace("*", "%")
                return and_(
                    getattr(partKlass, field).like(value),
                    condition,
                    self.inventory.c.Last == 1,
                )
        ...
        for machine in self.getMachinesOnly(ctx, pattern):
            ...
            tmp.append(
                self.getLastMachineInventoryPart(
                    ctx, "Bios", {"hostname": machine.Name}
                )
            )
            tmp.append(
                self.getLastMachineInventoryPart(
                    ctx, "Hardware", {"hostname": machine.Name}
                )
            )
    ...
    def getMachinesByDict(self, ctx, table, params):
        ...
        result = (
            query.group_by(self.machine.c.Name)
    ...
    def getMachinesBy(self, ctx, table, field, value, onlyName=True):
        ...
        result = (
            ...
            .add_column(func.min(self.inventory.c.Date))
            .select_from(
                self.machine.join(haspartTable)
                .join(
                    self.inventory,
                    getattr(haspartTable.c, "inventory") == self.inventory.c.id,
                )
                .join(partTable)
            )
        )
        ...
        result = (
            result.group_by(self.machine.c.Name)
            .group_by(haspartTable.c.machine)
            .order_by(haspartTable.c.machine)
            .order_by(desc("inventoryid"))
            .order_by(haspartTable.c.inventory)
    ...
    def getComputersOS(self, uuids):
        ...
        q = (
            ...
            .select_from(
                self.machine.join(
                    self.table["hasHardware"],
                    self.machine.c.id == self.table["hasHardware"].c.machine,
                )
                .join(
                    self.table["Hardware"],
                    self.table["Hardware"].c.id == self.table["hasHardware"].c.hardware,
                ...
                .join(
                    ...
                    self.table["Inventory"].c.id
                    == self.table["hasHardware"].c.inventory,
                )
            )
        )
        q = q.filter(
    ...
    def getMachineByInventory(self, ctx, inventory):
        ...
        if "Network" in inventory:
            logging.getLogger().debug(
                "Try to associate a MAC address to an existing machine"
            )
                ...
                if "MACAddress" in network:
                    ...
                    if "Virtual" not in network or network["Virtual"] == "0":
                        logging.getLogger().info(
                            "Trying to associate to an existing machine using MAC %s"
                            % mac
                        )
                        machines = self.getMachinesBy(
                            ctx, "Network", "MACAddress", mac, False
                        )
                        ...
                        if len(machines) == 1:
                            ...
                            message = (
                                "Match found using MAC %s! UUID: %s Hostname: %s"
                                % (mac, uuid, name)
                            )
                            ...
                            logging.getLogger().warn(
                                "Cannot resolve machine name: duplicate MAC address (%s)"
                                % mac
                            )
                        else:
                            logging.getLogger().debug(
                                "Cannot find a machine with MAC %s" % mac
                            )
                    else:
                        logging.getLogger().info(
                            "Skipping MAC %s, interface is marked as being virtual"
                            % mac
                        )
    ...
    def getMachineByHostnameAndMacs(self, ctx, hostname, macs):
        ...
        if machine_name == hostname:
            return machine_uuid
        self.logger.warn(
            "I can't get any UUID for machine %s and macs %s" % (hostname, macs)
        )
        return None
    ...
    def getValues(self, table, field):
        ...
        result = (
            session.query(partKlass)
            .add_column(getattr(partKlass, field))
            .limit(MAX_REQ_NUM)
        )
        session.close()
    ...
    def getValuesFuzzy(self, table, field, fuzzy_value):
        ...
        result = (
            session.query(partKlass)
            .add_column(getattr(partKlass, field))
            .filter(getattr(partKlass, field).like("%" + fuzzy_value + "%"))
            .limit(MAX_REQ_NUM)
            .all()
        )
        session.close()
    ...
    def getLastMachineInventoryPart2(self, ctx, part, params):
        ...
        if part == "Summary":
            ...
            if bios:
                ...
                if (
                    hardware
                    and hardware["Type"] is not None
                    and str(hardware["Type"]) != "0"
                ):
                ...
                # May be NULL, or "0" (int ?)
                    ...
                    bios
                    and bios["TypeMachine"] is not None
                    and str(bios["TypeMachine"]) != "0"
                ):
            ...
            if len(custom):
                fields.append(
                    [
                        "Description",
                        ["description", "text", custom[0][1][0]["Comments"]],
                    ]
                )
        ...
        if part == "Processors":
            ...
            if hardware:
                ...
                if hardware["ProcessorFrequency"]:
                    fields.append(
                        ["Frequency", hardware["ProcessorFrequency"] + " MHz"]
                    )
            ret.append(fields)
        ...
        if part == "Controllers":
            # fields = []
            controllers = Inventory().getLastMachineInventoryPart(
                ctx, "Controller", params
            )
            if controllers:
        ...
        if part == "GraphicCards":
            graphicCards = Inventory().getLastMachineInventoryPart(
                ctx, "VideoCard", params
            )
            if graphicCards:
                ...
                for item in Storages[0][1]:
                    ...
                    if item["DriveLetter"] and item["VolumeName"]:
                        z.insert(
                            ...
                            [
                                "Name",
                                item["DriveLetter"] + " " + str(item["VolumeName"]),
                            ],
                        )
                    if item["TotalSpace"]:
    ...
    def __lastMachineInventoryPartQuery(self, session, ctx, part, params):
        ...
        noms = self.config.getInventoryNoms()
        select_from = (
            haspartTable.join(self.inventory).join(partTable).outerjoin(self.machine)
        )
        ...
        if (
            "software_filter" in params
            and params["software_filter"]
            and part == "Software"
        ):
        ...
        if (
            "hide_win_updates" in params
            and params["hide_win_updates"]
            and part == "Software"
        ):
            result = result.filter(
                not_(
                    partKlass.ProductName.op("regexp")("KB[0-9]+(-v[0-9]+)?(v[0-9]+)?")
                )
            )
            ...
            for where in params["where"]:
                if hasattr(partTable.c, where[0]):
                    if isinstance(where[1], list):
                        result = result.filter(
                            getattr(partTable.c, where[0]).in_(where[1])
                        )
                    else:
                        result = result.filter(
    ...
    def addMachine(self, name, ip, mac, netmask, comment=None, location_uuid=None):
        ...
        if not (isUUID(location_uuid)):
            ...
            msg = (
                "inventory.addMachine() : tried to add a computer on an invalid entity (%s), linking it to the root entity"
                % location_uuid
            )
        ...
        query = (
            session.query(InventoryTable)
            .select_from(
                self.inventory.join(self.table["hasNetwork"]).join(self.machine)
            )
            .filter(self.machine.c.Name == name)
        )
        for inv in query:
            inv.Last = 0
            session.add(inv)
        ...
        if location_uuid is not None:
            query = (
                ...
                .filter(self.table["Entity"].c.id == fromUUID(location_uuid))
                .first()
            )
    ...
    def delMachine(self, uuid):
        ...
        i = (
            session.query(InventoryTable)
            .select_from(self.inventory.join(self.table["hasInventory"]))
            .filter(self.table["hasInventory"].c.machine == m.id)
            .first()
        )
        for item in self.config.getInventoryParts():
            #            tk = self.klass[item]
        ...
        if i is None:
            self.logger.error(
                "delMachine : try to remove a None Inventory! (%s)" % uuid
            )
        else:
            session.delete(i)
        session.delete(m)
    ...
    def updateNameWithHardwareHost(self, uuid, name):
        ...
        query1 = query1.select_from(
            ...
            .join(
                self.machine, self.machine.c.id == self.table["hasHardware"].c.machine
            )
            .join(
                self.table["Inventory"],
                self.table["Inventory"].c.id == self.table["hasHardware"].c.inventory,
    ...
    def confFileRule(self):
        cfgfile = os.path.join(
            mmcconfdir, "medulla", "inventory-server", "inventory-server.ini"
        )
    ...
    def operatorType(self):
        listType = []
        OcsNGMapXml = mmcconfdir + "/medulla/inventory-server/OcsNGMap.xml"
    ...
    def parse_file_rule(self, param=None):
        ...
        # conffile = mmcconfdir + '/medulla/inventory-server/entities-rules'
        actif = False
        tab = []
        ret = {}
        conffile = self.confFileRule()
        ...
        for line in open(conffile):
            ...
            try:
                ...
                entitieslist1 = [
                    x for x in entitieslist if re.match("^[a-zA-Z0-9]{3,64}$", x)
                ]
    ...
    def getMachineByOsLike(self, session, ctx, osnames, count=0):
        ...
        # OS (used for reporting, per example...)
            ...
            os_filter = [
                self.table["Hardware"].c.OperatingSystem.like("%" + osname + "%")
    ...
    def getMachineByType(self, session, ctx, types, count=0):
        ...
        query = query.select_from(
            ...
            .join(
                self.table["hasBios"],
                (self.machine.c.id == self.table["hasBios"].c.machine),
            )
            .join(
                self.table["Hardware"],
                self.table["Hardware"].c.id == self.table["hasHardware"].c.hardware,
            ...
            .join(
                self.table["Bios"],
                self.table["Bios"].c.id == self.table["hasBios"].c.bios,
            )
            .join(
                self.table["hasEntity"],
                (self.table["hasEntity"].c.machine == self.machine.c.id),
        ...
        typeField = func.if_(
            func.isnull(hardwareType) | (hardwareType.in_(["", "0"])),
            biosType,
            hardwareType,
        )
    ...
    def getAllSoftwaresImproved(
        ...
        query = query.select_from(
            self.software.join(
                ...
                self.table["hasSoftware"].c.software == self.software.c.id,
            )
            .join(
                self.table["hasEntity"],
                self.table["hasEntity"].c.inventory
                == self.table["hasSoftware"].c.inventory,
            )
            .join(
                self.table["Inventory"],
    ...
    def getUserLocations(self, userid, with_level=False):
        ...
        def __addChildren(session, rootid, level):
            ...
            q = (
                session.query(self.klass["Entity"])
                .filter(self.table["Entity"].c.parentId == rootid)
                .order_by(self.table["Entity"].c.Label)
            )
            for entity in q:
                if entity.id != 1:
                    if with_level:
        ...
        if userid != "root":
            q = (
                session.query(self.klass["Entity"])
                .select_from(
                    self.table["Entity"].join(self.userentities).join(self.user)
                )
                .filter(self.user.c.uid == userid)
                .order_by(self.table["Entity"].c.Label)
            )
        else:
            q = (
                session.query(self.klass["Entity"])
                .filter(self.table["Entity"].c.id == 1)
                .order_by(self.table["Entity"].c.Label)
            )
        level = 1
        for entity in q:
    ...
    def getLocationsFromPathString(self, location_paths):
        ...
        for name in location_paths:
            try:
                q = (
                    session.query(self.klass["Entity"])
                    .filter(self.table["Entity"].c.Label == name)
                    .one()
                )
    ...
    def getLocationParentPath(self, loc_uuid):
        ...
        en = (
            session.query(self.klass["Entity"])
            .filter(self.table["Entity"].c.id == en_id)
            .first()
        )
        ...
        while parent_id != en_id:
            ...
            en = (
                session.query(self.klass["Entity"])
                .filter(self.table["Entity"].c.id == parent_id)
                .first()
            )
    ...
    def getUsersInSameLocations(self, userid, locations=None):
        ...
        if locations:
            ...
            q = (
                ...
                .filter(self.userentities.c.fk_Entity.in_(inloc))
                .filter(self.user.c.uid != userid)
                .distinct()
                .all()
            )
            ...
            .filter(
                and_(
                    (
                        ...
                        - func.to_days(self.klass["Inventory"].Date)
                    )
                    <= days,
    ...
    def getInventoryHistory(self, days, only_new, pattern, max, min):
        ...
        results = self.__getInventoryHistory(
            session, days, only_new, pattern, max, min
        ).limit(max - min)
        ...
        for res in results:
            ...
            for prev in results:
                if (
                    res[1].id == prev[1].id
                    and res[0].Date >= prev[0].Date
                    and res[0].Time > prev[0].Time
                ):
                    newMachine = False
                    break
    ...
    def countComputerInventoryHistory(self, ctx, params):
        ...
        count = (
            session.query(self.klass["Inventory"])
            .filter(self.klass["Inventory"].id.in_([l.id for l in count]))
            .count()
        )
        session.close()
    ...
    def getComputerInventoryDiff(self, ctx, params):
        ...
        session.close()
        self.logger.debug(
            "Comparing inventory id %d and %d"
        ...
        if self.__isIdenticalInventory(
            ...
            removed_elements,
            exclude={
                "Hardware": ["ProcessorFrequency", "id"],
            },
        ):
            return [[], []]
        ...
        ret = {
            "green": int(
                self.__machinesOnlyQuery(
                    ctx,
                    pattern={"green": {"date_mod": date_mod, "orange": orange}},
                    session=session,
                    count=True,
                )
            ),
            "orange": int(
                self.__machinesOnlyQuery(
                    ctx,
                    pattern={
                        "orange": {"date_mod": date_mod, "orange": orange, "red": red}
                    },
                    session=session,
                    count=True,
                )
            ...
            "red": int(
                self.__machinesOnlyQuery(
                    ctx,
                    pattern={"red": {"date_mod": date_mod, "red": red}},
                    session=session,
                    count=True,
                )
            ),
    ...
    def __isIdenticalInventory(self, added, removed, exclude={}):
        ...
        for part_key in a_keys:
            for added_part in added[part_key]:
                ...
                # elements
                if (
                    len(a_elem_keys - r_elem_keys) != 0
                    and len(r_elem_keys - a_elem_keys) != 0
                ):
                    return False
    ...
    def update_owner(self, machine_uuid):
        ...
        logging.getLogger().info(
            "Trying to update the owner (machine UUID%s)" % (machine_id)
        )
        ...
        if hardware:
            ...
            session.flush()
            logging.getLogger().info(
                "New owner of machine UUID%s is %s" % (machine_id, hardware.Owner)
            )
...
def orderIpAdresses(netiface):
    ...
    for iface in netiface:
        ...
        if (
            "IP" in iface
            and iface["IP"]
            and iface["MACAddress"] != "00-00-00-00-00-00-00-00-00-00-00"
        ):
...
class InventoryCreator(Inventory):
    ...
    def createNewInventory(
        ...
        try:
            ...
            from medulla.inventoryserver.utils import canDoInventory
            ...
            if machine_exists:
                if setLastFlag:
                    result = (
                        session.query(InventoryTable)
                        .select_from(
                            self.inventory.join(self.table["hasEntity"]).join(
                                self.machine
                            )
                        )
                    ...
                    try:
                        ...
                        if table in OcsMapping().nomenclatures:
                            for nom in OcsMapping().nomenclatures[table]:
                                ...
                                new_entry = {}
                                for field in entry:
                                    if isinstance(field, tuple) and field[0] == nomName:
...
class InventoryNetworkComplete:
    ...
    def _get_networking(self):
        ...
        netmask = fcntl.ioctl(n_sck, self.SIOCGIFNETMASK, struct.pack("256s", iface))[
            20:24
        ]
