...
def checkPool():
    ...
    try :
        ...
        if pool._max_overflow > -1 and pool._overflow >= pool._max_overflow :
            logger.error('dyngroup plugin: CHECK: NOK: timeout then overflow (%d vs. %d) detected in SQL pool : check your network connectivity !' % (pool._overflow, pool._max_overflow))
            pool.dispose()
            pool = pool.recreate()
            ret = False
    except Exception, e:
        logger.warn('dyngroup plugin: CHECK: NOK: got the following error : %s' % (e))
        ret = False
    else:
        logger.debug('dyngroup plugin: CHECK: OK, pool is (%d / %d)' % (pool._overflow, pool._max_overflow))
    return ret

def checkStatus():
    if checkPool():
        logger.info('dyngroup plugin: CHECK: OK')

def scheduleCheckStatus(interval):
    """ periodicaly check our status stats """
    logger.debug('dyngroup plugin: CHECK: Sleeping')
...
def awakeCheckStatus(interval):
    logger.debug('dyngroup plugin: CHECK: Starting')
    checkStatus()
