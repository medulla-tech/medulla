...
def putPackageDetail(package, need_assign=True):
    ...
    confjson = {
        ...
        "inventory": {
            ...
            "queries": {
                "Qversion": (
                    package["Qversion"].decode("utf-8")
                    if isinstance(package["Qversion"], bytes)
                    else package["Qversion"]
                ),
                "Qvendor": (
                    package["Qvendor"].decode("utf-8")
                    if isinstance(package["Qvendor"], bytes)
                    else package["Qvendor"]
                ),
                "boolcnd": package["boolcnd"],
                "Qsoftware": (
                    package["Qsoftware"].decode("utf-8")
                    if isinstance(package["Qsoftware"], bytes)
                    else package["Qsoftware"]
                ),
            },
        },
    }
