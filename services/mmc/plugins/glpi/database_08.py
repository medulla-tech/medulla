...
class Glpi08(DyngroupDatabaseHelper):
    ...
    def getRestrictedComputersList(self, ctx, min = 0, max = -1, filt = None, advanced = True, justId = False, toH = False):
        ...
        if justId:
            ret = map(lambda m: self.getMachineUUID(m), query.group_by([self.machine.c.name, self.machine.c.domains_id]).order_by(asc(self.machine.c.name)))
        elif toH:
            ret = map(lambda m: m.toH(), query.group_by([self.machine.c.name, self.machine.c.domains_id]).order_by(asc(self.machine.c.name)))
        else:
            if filt.has_key('get'):
                ret = self.__formatMachines(query.group_by([self.machine.c.name, self.machine.c.domains_id]).order_by(asc(self.machine.c.name)), advanced, filt['get'])
            else:
                ret = self.__formatMachines(query.group_by([self.machine.c.name, self.machine.c.domains_id]).order_by(asc(self.machine.c.name)), advanced)
        session.close()
        return ret
