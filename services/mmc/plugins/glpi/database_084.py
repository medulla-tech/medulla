...
class Glpi084(DyngroupDatabaseHelper):
    ...
    def _machineobjectdymresult(self, ret, encode='iso-8859-1'):
        ...
        try:
                    ...
                    try:
                        if getattr(ret, keynameresult) is None:
                            ...
                            if "class" in typestr:
                                try:
                                    resultrecord[keynameresult] = (
                                        float(getattr(ret, keynameresult))
                                        if 'decimal.Decimal' in typestr
                                        else str(getattr(ret, keynameresult))
                                    )
                                except:
                                    self.logger.warning(f"type class {typestr} no used for key {keynameresult}")
                                    resultrecord[keynameresult] = ""
                            elif isinstance(getattr(ret, keynameresult), datetime.datetime):
            ...
            self.logger.error(f"We encountered the error {str(e)}")
