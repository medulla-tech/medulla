...
class Glpi090(DyngroupDatabaseHelper):
    ...
    def activate(self, config = None):
        ...
        try:
            ...
            setattr(Glpi090, "decode", decode_utf8)
            setattr(Glpi090, "encode", encode_utf8)
        except:
            self.logger.warn("Your database is not in utf8, will fallback in latin1")
            setattr(Glpi090, "decode", decode_latin1)
            setattr(Glpi090, "encode", encode_latin1)
	try:
            self._glpi_version = self.db.execute('SELECT version FROM glpi_configs').fetchone().values()[0].replace(' ', '')
	except OperationalError:
            self._glpi_version = self.db.execute('SELECT value FROM glpi_configs WHERE name = "version"').fetchone().values()[0].replace(' ', '')
...
class Machine(object):
    ...
    def to_a(self):
        owner_login, owner_firstname, owner_realname = Glpi090().getMachineOwner(self)
        return [
            ['name',self.name],
            ...
            ['uuid',Glpi090().getMachineUUID(self)]
        ]
