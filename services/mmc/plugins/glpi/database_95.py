...
class Glpi95(DyngroupDatabaseHelper):
    ...
    def initMappers(self):
        ...
        # autoload = True)
        # self.mapper_registry.map_imperatively(OCSLinks, self.ocslinks)
    ...
    def getLastMachineInventoryFull(self, uuid):
        """Name dynamically generated before called by frontend side.
        Correspont to the tabs names on glpi computer inventory parts.
        """
        session = self.session_factory()
    ...
    def get_machine_with_update(self, session, kb):
        sqlrequest = """
            SELECT
                ...
                glpi_computers.date_creation AS input_computer
            FROM
                glpi_computers
                    INNER JOIN
                glpi_items_softwareversions ON glpi_computers.id = glpi_items_softwareversions.items_id and glpi_items_softwareversions.itemtype="Computer"
                    INNER JOIN
                glpi_softwares ON glpi_softwares.id = glpi_items_softwareversions.softwareversions_id
                    INNER JOIN
