...
class Glpi110(DyngroupDatabaseHelper):
    """
    Singleton Class to query the glpi database in version >= 11.0
    ...
    def try_activation(self, config):
        ...
        try:
            self._glpi_version = list(
                self.db.execute(
                    "SELECT version FROM glpi_configs").fetchone().values()
            )[0].replace(" ", "")
    ...
    def activate(self, config=None):
        ...
        try:
            ...
            setattr(Glpi110, "decode", decode_utf8)
            setattr(Glpi110, "encode", encode_utf8)
        except:
            self.logger.warn(
                "Your database is not in utf8, will fallback in latin1")
            setattr(Glpi110, "decode", decode_latin1)
            setattr(Glpi110, "encode", encode_latin1)
    ...
    def initMappers(self):
        ...
        for i in self.devices:
            setattr(self, i, Table("glpi_%s" %
                    i, self.metadata, autoload_with=self.db))
            j = self.getTableName(i)
            exec("class %s(DbTOA): pass" % j)
            self.mapper_registry.map_imperatively(eval(j), getattr(self, i))
            self.klass[i] = eval(j)
            ...
            setattr(
                ...
                Table(
                    "glpi_items_%s" % i,
                    self.metadata,
                    Column("items_id", Integer, ForeignKey(
                        "glpi_computers_pulse.id")),
        ...
        self.rule_criterias = Table(
            "glpi_rulecriterias", self.metadata, autoload_with=self.db)
        self.mapper_registry.map_imperatively(RuleCriterion, self.rule_criterias)

        self.rule_actions = Table(
            "glpi_ruleactions", self.metadata, autoload_with=self.db)
        ...
        # processor
        self.processor = Table("glpi_deviceprocessors",
                               self.metadata, autoload_with=self.db)
        ...
        self.computerProcessor = Table(
            ...
            Column("items_id", Integer, ForeignKey("glpi_computers_pulse.id")),
            Column(
                "deviceprocessors_id", Integer, ForeignKey(
        ...
        self.memory = Table(
            ...
            self.metadata,
            Column(
                "devicememorytypes_id", Integer, ForeignKey(
                    "glpi_devicememorytypes.id")
            ),
            autoload_with=self.db,
        )
        self.mapper_registry.map_imperatively(Memory, self.memory)

        self.memoryType = Table("glpi_devicememorytypes",
                                self.metadata, autoload_with=self.db)
        ...
        self.computerMemory = Table(
            ...
            self.metadata,
            Column("items_id", Integer, ForeignKey("glpi_computers_pulse.id")),
            Column("devicememories_id", Integer,
        ...
        self.interfaceType = Table(
            "glpi_interfacetypes", self.metadata, autoload_with=self.db)
        ...
        try:
            ...
            self.fusionantivirus = Table(
                ...
                self.metadata,
                Column("computers_id", Integer, ForeignKey(
                    "glpi_computers_pulse.id")),
                Column(
                    "manufacturers_id", Integer, ForeignKey(
        ...
        if self.fusionantivirus is not None:  # Fusion is not installed
            try:
                ...
                self.fusionagents = Table(
                    ...
                    self.metadata,
                    Column(
                        "computers_id", Integer, ForeignKey(
                            "glpi_computers_pulse.id")
                    ),
                    autoload_with=self.db,
                )
                ...
                self.fusionagents = Table(
                    ...
                    self.metadata,
                    Column(
                        "entities_id", Integer, ForeignKey(
                            "glpi_computers_pulse.id")
                    ),
                    autoload_with=self.db,
                )
        ...
        self.ipnetworks = Table(
            "glpi_ipnetworks", self.metadata, autoload_with=self.db)
        ...
        self.ipaddresses_ipnetworks = Table(
            ...
            self.metadata,
            Column("ipaddresses_id", Integer,
                   ForeignKey("glpi_ipaddresses.id")),
        ...
        self.ipaddresses = Table(
            "glpi_ipaddresses", self.metadata, autoload_with=self.db)
        ...
        self.networknames = Table(
            "glpi_networknames", self.metadata, autoload_with=self.db)
        self.mapper_registry.map_imperatively(
            NetworkNames,
            self.networknames,
            properties={
        ...
        self.networkports = Table(
            "glpi_networkports", self.metadata, autoload_with=self.db)
        self.mapper_registry.map_imperatively(
            NetworkPorts,
            self.networkports,
            properties={
        ...
        self.machine = Table(
            ...
            Column(
                "operatingsystems_id", Integer, ForeignKey(
        ...
        self.manufacturers = Table(
            "glpi_manufacturers", self.metadata, autoload_with=self.db)
        ...
        self.inst_software = Table(
            ...
            Column("items_id", Integer, ForeignKey("glpi_computers_pulse.id")),
            Column(
                "softwareversions_id", Integer, ForeignKey(
        ...
        try:
            self.regcontents = Table(
                ...
                Column(
                    ...
                    Integer,
                    ForeignKey(
                        "glpi_plugin_fusioninventory_collects_registries.id"),
                ),
                autoload_with=self.db,
            )
        except NoSuchTableError:
            self.regcontents = Table(
                ...
                Column(
                    ...
                    Integer,
                    ForeignKey(
                        "glpi_plugin_glpiinventory_collects_registries.id"),
                ),
                autoload_with=self.db,
            )
        ...
        self.view_computers_items_printer = Table(
            ...
            self.metadata,
            Column("id", Integer, primary_key=True),
            Column("items_id_peripheral", Integer, ForeignKey("glpi_printers.id")),
            Column("items_id_asset", Integer, ForeignKey(
                "glpi_computers_pulse.id")),
            autoload_with=self.db,
        )
        ...
        self.view_computers_items_peripheral = Table(
            ...
            self.metadata,
            Column("id", Integer, primary_key=True),
            Column("items_id_peripheral", Integer, ForeignKey("glpi_peripherals.id")),
            Column("items_id_asset", Integer, ForeignKey(
                "glpi_computers_pulse.id")),
            autoload_with=self.db,
        )
        self.mapper_registry.map_imperatively(Computersviewitemsperipheral,
               self.view_computers_items_peripheral)
        ...
        self.glpi_view_peripherals_manufacturers = Table(
            ...
            Column("id", Integer, primary_key=True),
            Column(
                "items_id", Integer, ForeignKey(
                    "glpi_peripherals.manufacturers_id")
            ),
            autoload_with=self.db,
        )
        self.mapper_registry.map_imperatively(Peripheralsmanufacturers,
               self.glpi_view_peripherals_manufacturers)
        ...
        # Peripherals items
        self.peripherals = Table(
            "glpi_peripherals", self.metadata, autoload_with=self.db)
        ...
        if self.config.filter_on != None:
            ...
            for filter_key, filter_values in list(self.config.filter_on.items()):
                ...
                if not filter_key in (
                    ...
                    self.logger.warn(
                        "dont know how to filter on %s" % (filter_key))
    ...
    def get_machines_list1(self, session, start, end, ctx):
        ...
        for regkey in regs:
            regkeyconf = getattr(master_config, regkey).split("|")[
                0].split("\\")[-1]
        ...
        query = query.filter(Machine.is_deleted == 0).filter(
        ...
        regquery = (
            ...
            .filter(
                and_(
                    ...
                    self.regcontents.c.computers_id.in_(
                        result["data"]["uuid"]),
                )
            )
            .all()
    ...
    def get_machines_list(self, session, start, end, ctx):
        ...
        if idmachine != "" or uuidsetup != "":
            ...
            for addcolumn in list_column_add_for_info:
                query = query.add_column(
                    getattr(Machine, addcolumn).label(addcolumn))
    ...
    def __getRestrictedComputersListQuery(
        ...
        if filt:
                ...
                if location is not None:
                    ...
                    if isinstance(location, list):
                        ...
                        query_filter = self.__addQueryFilter(
                            query_filter, (self.machine.c.entities_id.in_(
                                locationids))
                        )
                    else:
                        ...
                        if locationid in locsid:
                            query_filter = self.__addQueryFilter(
                                query_filter, (self.machine.c.entities_id ==
                                               locationid)
                            )
                        else:
                            self.logger.warn(
            ...
            if "name" in filt:
                query = query.filter(
                    self.machine.c.name.like("%" + filt["name"] + "%"))
                                ...
                                and_(
                                    ...
                                    not_(
                                        FusionAntivirus.computers_id.in_(
                                            self.getMachineIdsNotInAntivirusRed(
                                                ctx),
                                        )
                                    ),
                                ),
    ...
    def __formatMachines(self, machines, advanced, get=None, empty_macs=False):
        ...
        for m in machines:
            ...
            if displayList:
                ...
                for regkey in regs:
                    regkeyconf = (
                        getattr(master_config, regkey).split(
                            "|")[0].split("\\")[-1]
                    )
                    try:
                        keyname, keyvalue = self.getMachineRegistryKey(
                            m, regkeyconf)
        ...
        query = (
            ...
            .select_from(
                self.machine.outerjoin(
                    self.regcontents).outerjoin(self.registries)
            )
        )
        query = query.filter(
    ...
    def getWarrantyEndDate(self, infocoms):
        ...
        if infocoms is not None and infocoms.warranty_date is not None:
            endDate = add_months(infocoms.warranty_date,
                    ...
                    l = [
                        ...
                        [
                            ...
                            self._get_network_type(
                                networkport.instantiation_type),
                        ],
                ...
                if diskfs not in ["rootfs", "tmpfs", "devtmpfs"]:
                    if disk is not None:
                        l = [
                            ...
                            [
                                "Size",
                                disk.totalsize and str(
                                    disk.totalsize) + " MB" or "",
                            ],
                            [
                                "Free Size",
                                disk.freesize and str(
                                    disk.freesize) + " MB" or "",
                            ],
                        ]
                        ret.append(l)
    ...
    def getLastMachineAntivirusPart(
        ...
        if count:
            ...
            for antivirus, manufacturerName in query:
                if antivirus:
                    l = [
                        ["Name", __getAntivirusName(
                            manufacturerName, antivirus.name)],
    ...
    def setGlpiEditableValue(self, uuid, name, value):
        ...
        self.logger.debug("%s: Set %s as new value for %s" %
                          (uuid, value, name))
        try:
            session = self.session_factory()
    ...
    def getLastMachineHistoryPart(
        ...
        if history_delta == "today":
            query = query.filter(self.logs.c.date_mod >
        ...
        if filt:
            ...
            clauses.append(self.logs.c.itemtype_link.in_(
    ...
    def getAllVersion4Software(self, session, ctx, softname, version=""):
        ...
        query = query.filter(
            or_(
                ...
                and_(
                    Software.is_recursive == 1, Software.entities_id.in_(
                        my_parents_ids)
                ),
            )
        )
        ...
        if version:
            query = query.filter(
                SoftwareVersion.name.like("%" + version + "%"))
    ...
    def getAllContactNums(self, ctx, filt=""):
        ...
        if filter != "":
            query = query.filter(
                self.machine.c.contact_num.like("%" + filt + "%"))
    ...
    def getAllManufacturers(self, ctx, filt=""):
        ...
        if filter != "":
            query = query.filter(
                self.manufacturers.c.name.like("%" + filt + "%"))
    ...
    def getAllTypes(self, ctx, filt=""):
        ...
        if filter != "":
            query = query.filter(
                self.glpi_computertypes.c.name.like("%" + filt + "%"))
    ...
    def getAllLocations(self, ctx, filt=""):
        ...
        if not hasattr(ctx, "locationsid"):
            complete_ctx(ctx)
        session = self.session_factory()
        query = session.query(Locations).select_from(
            self.locations.join(self.machine))
        query = self.__filter_on(
            query.filter(self.machine.c.is_deleted == 0).filter(
                self.machine.c.is_template == 0
            )
        ...
        if filter != "":
            query = query.filter(
                self.locations.c.completename.like("%" + filt + "%"))
    ...
    def getAllRegistryKeyValue(self, session, ctx, keyregister, value):
        ...
        query = self.__filter_on_entity(query, ctx)
        query = query.filter(
            self.registries.c.key.like("%" + keyregister + "%"))
        ret = query.all()
        session.close()
        return ret
                    ...
                    try:
                        if getattr(ret, keynameresult) is None:
                            ...
                            if "class" in typestr:
                                ...
                                if isinstance(
                                    getattr(
                                        ret, keynameresult), datetime.datetime
                                ):
    ...
    def getMachineByUuidSetup(self, uuidsetupmachine):
        ...
        session = self.session_factory()
        ret = session.query(Machine).filter(and_(Machine.uuid.like(
            uuidsetupmachine), Machine.is_deleted == 0, Machine.is_template == 0)).first()
        session.close()
        return self._machineobject(ret)
    ...
    def getMachineIdsNotInAntivirusRed(self, ctx):
        ...
        query1 = __computersListQ(
            ctx, dict(filt, **{"antivirus": "green"}), session)
        query2 = __computersListQ(
            ctx, dict(filt, **{"antivirus": "orange"}), session)
    ...
    def addRegistryCollect(self, session, full_key, key_name):
        ...
        if hasattr(registry, "plugin_fusioninventory_collects_id"):
            ...
            self.logger.error(
                "No valid collects_id field found in Registries table.")
            return False
    ...
    def addRegistryCollectContent(self, session, computers_id, registry_id, key, value):
        ...
        query = session.query(RegContents).filter_by(
            computers_id=computers_id, key=key)
    ...
    def get_ancestors(self, session, uuid):
        ...
        query = (
            session.query(Entities.ancestors_cache).filter(
                Entities.id == id).first()
        )
    ...
    def getComputerFilteredByCriterion(self, session, ctx, criterion, values):
        ...
        if criterion == "Computer name":
            ...
            query = query.join(
                RegContents, RegContents.computers_id, Machine.id)
            ...
            query = query.join(
                OsSp, OsSp.id == Machine.operatingsystemservicepacks_id)
        ...
        query = query.filter(
            and_(Machine.is_deleted == 0, Machine.is_template == 0))
        response = query.all()
    ...
    def get_count_installed_updates_by_machines(self, session, ids):
        ...
        ids = "(%s)" % ",".join(
            [id for id in ids if id != ""]).replace("UUID", "")
