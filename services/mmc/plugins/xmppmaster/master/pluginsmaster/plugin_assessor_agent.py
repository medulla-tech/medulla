...
def action(objectxmpp, action, sessionid, data, msg, ret, dataobj):
    ...
    try:
        MessagesAgentFromChatroomConfig(
            objectxmpp, action, sessionid, data, msg, ret, dataobj
        )
    except Exception:
...
def MessagesAgentFromChatroomConfig(
    objectxmpp, action, sessionid, data, msg, ret, dataobj
):
    """
    ...
    if data["machine"].split(".")[0] in objectxmpp.assessor_agent_showinfomachine:
        ...
        logger.info(
            "showinfomachine = %s in file /etc/mmc/plugins/assessor_agent.ini(.local)"
            % (host)
        )
    else:
        showinfomachine = False
        logger.info(
            "gives showinfomachine in /etc/mmc/plugins/assessor_agent.ini(.local)"
    ...
    try:
        try:
            ...
            msglog = (
                ...
                "\nserverip = %s configuration error"
                % (objectxmpp.assessor_agent_serverip)
            )
            msglog1 = (
    ...
    if not os.path.isfile(pathfileconf):
        ...
        if Config.has_section("parameters"):
            ...
            if Config.has_option("parameters", "serverip"):
                ...
                logger.error(
                    "see parameter [serverip] " "missing in file : %s " % pathfileconf
                )
                objectxmpp.assessor_agent_errorconf = True
