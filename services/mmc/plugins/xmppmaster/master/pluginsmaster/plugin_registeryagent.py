    ...
    try:
        ...
        if "action" in data and data["action"] in ["infomachine", "registeryagent"]:
            ...
            if machine:
                ...
                try:
                    ...
                    if result[0] is None or result[0] != macadressstr:
                        if result[0] is not None:
                            logger.warning(
                                "The is a difference in the database between \n"
                                "the %s 's table: %s"
                                "and the network's table %s"
                ...
                if machine["enabled"] == 1:
                    ...
                    if data["agenttype"] == "relayserver":
                        ...
                        notify_ars_to_recording(
                            "recording_case1", str(msg["from"]), xmppobject
                        )
                        XmppMasterDatabase().update_Presence_Relay(
                            str(msg["from"]), presence=1
                        )
                    if (
                    ...
                    if (
                        ...
                        if data["agenttype"] != "relayserver":
                            ...
                            logger.info(
                                "result test_consolidation_inventory  %s %s"
                                % (btestfindcomputer, idglpimachine)
                            )
            ...
            if idmachine != -1:
                ...
                if showinfobool:
                    XmppMasterDatabase().setlogxmpp(
                        "configuration guacamole for machine %s"
                        % (data["information"]["info"]["hostname"]),
                        "warn",
                        sessionid,
                        -1,
...
def notify_ars_to_recording(notify, to, xmppobject):
    datasend = {
        "action": "notify_information",
        "ret": 0,
        "sessionid": getRandomName(5, "notify_information"),
        "data": {"notify": notify},
    }
    xmppobject.send_message(mto=to, mbody=json.dumps(datasend), mtype="chat")
...
def read_conf_remote_registeryagent(xmppobject):
    ...
    for regexpconf in blacklisted_mac_addresseslist:
        try:
            logger.debug(
                "We are backlisting the following mac address: %s" % regexpconf
            )
            xmppobject.blacklisted_mac_addresses.append(re.compile(regexpconf))
        except Exception:
            logger.error("\n%s" % (traceback.format_exc()))
            logger.error(
                "An error occured while blacklisting this mac address %s. We ignore it."
                % regexpconf
            )
...
def test_consolidation_inventory(
    ...
    if data["uuid_serial_machine"] != "":
        if showinfobool:
            logger.info(
                "***** Finding uuid from GLPI computer id for uuid setup machine"
            )
    else:
        if showinfobool:
