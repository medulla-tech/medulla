...
class manage_grafana:
    ...
    def configuration(self):
        ...
        if config.has_option('grafana_api', 'api_key'):
            self.api_configuration['api_key'] = config.get('grafana_api',
                                                           'api_key')
        else:
        ...
        if config.has_option('grafana_api', 'url'):
            self.api_configuration['url'] = config.get('grafana_api',
                                                       'url')
        else:
            self.api_configuration['url'] = 'http://localhost:3000'

        logger.debug("API configuration: %s" % self.api_configuration)
    ...
    def grafanaGetDashboardForHostname(self):
        ...
        try:
            ...
            url = "%s/api/dashboards/uid/%s" % (self.api_configuration['url'],
                                                self.hostname)
        except KeyError:
    ...
    def grafanaWriteDashboardForHostname(self, dashb_json):
        ...
        try:
            ...
            headers = {'Authorization': 'Bearer %s'
                       % self.api_configuration['api_key']}
            url = "%s/api/dashboards/db" % self.api_configuration['url']
        except KeyError:
            logger.error("API key or url is not defined")
            return None
        # Send request to grafana API
    ...
    def grafanaGetPanelImage(self, rendering_url, panel_title):
        ...
        if panel_id:
            img = "%s/%s/%s?panelId=%s&width=1000&height=500" % (rendering_url,
                                                                 self.hostname,
                                                                 self.hostname,
                                                                 panel_id)
