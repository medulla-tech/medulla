...
class MUCBot(slixmpp.ClientXMPP):
    ...
    def iqsendpulse(self, to, datain, timeout):
        ...
        try:
            ...
            try:
                ...
                error_text = "error for IQ [%s] data [%s]" % (
                    struccmd["action"],
                    struccmd["data"],
                )
            except Exception:
        ...
        try:
            ...
            try:
                ...
                if errortext == "User session not found":
                    logging.error(
                        "User session not found on iq %s second" % (end_iq - start_iq)
                    )
                    logging.error(
                        "Making the machine %s in status OFF in machine table"
                        % (str(to))
                    )
                    error_text = "%s\\n(Making the machine (%s) in status OFF)" % (
                        error_text,
                        str(to),
                    )
                    XmppMasterDatabase().changStatusPresenceMachine(to, enable="0")
                elif (
                    errortext == "No module is handling this query"
                    and conditionxmppiq == "service-unavailable"
                ):
                    logging.error(
                        "The IQ query is sent to a JID that cannot handle the query."
                    )
                    error_text = (
                ...
                if (end_iq - start_iq) < ref_time:
                    logging.error(
                        "direct Time out Error on iq %s" % (end_iq - start_iq)
                    )
    ...
    def pluginaction(self, rep):
        if "sessionid" in rep.keys():
            ...
            if (
                "shell" in sessiondata.getdatasession().keys()
                ...
                for t in results:
                    ...
                    if computer != None:
                        ...
                        if (
                            ...
                            and machine["uuid_inventorymachine"] != None
                        ):
                            logger.debug(
    ...
    def callpluginmaster(self, msg):
        try:
            ...
            if (
                dataobj.has_key("action")
                and dataobj["action"] != ""
                and dataobj.has_key("data")
            ):
                if dataobj.has_key("base64") and (
                    (isinstance(dataobj["base64"], bool) and dataobj["base64"] == True)
                    or (
