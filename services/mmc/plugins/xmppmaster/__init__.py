...
class RpcProxy(RpcProxyI):
    ...
    def get_remote_log_machine(self, params, ctx=None):
        ...
        agenttype = params.get("agenttype",0)
        ...
        if agenttype:
            ...
            jidreduit = params.get("jid", None)
            # Extraire la partie "cn" (sans la ressource)
            if jidreduit:
                # cn = jidreduit.split('/')[0]  # Séparer par '/' et prendre la première partie
                # params["cn"] = cn
                params["cn"] = jidreduit
            ...
            if id:
                ...
                logger.error("Relay SERVEUR id missing impossible d'identifier le relay.")
                return False
            is_present = XmppMasterDatabase().getPresenceid(id)
            msg=f"La relay '{id}' n'est pas présente sur le bus XMPP."
        else:
            ...
            msg=f"La machine avec UUID '{machine_uuid}' n'est pas présente sur le bus XMPP."

        if not is_present:
            logger.error(msg)
            return False
        params["presencemachinexmpp"] = 1
        # --- Vérification du JID ---
        ...
        # logger.debug(f"namemachine {namemachine}")
    ...
    def getPresenceid(self, id, enabled=1):
        return XmppMasterDatabase().getPresenceuuids(id, enabled)
