...
DELIMITER $$
USE `xmppmaster`$$
CREATE DEFINER=`root`@`localhost` TRIGGER `xmppmaster`.`up_white_list_AFTER_INSERT` AFTER INSERT ON `up_white_list` FOR EACH ROW
BEGIN
...
-- ----------------------------------------------------------------------
-- Apres Insertion up_black_list_BEFORE_INSERT
-- ----------------------------------------------------------------------

-- des que l'on insere package  dans la liste black.
-- on doit supprimer tout les deployement pas encore lancer dans msc et deploy
DROP TRIGGER IF EXISTS `xmppmaster`.`up_black_list_BEFORE_INSERT`;

DELIMITER $$
USE `xmppmaster`$$
CREATE DEFINER=`root`@`localhost` TRIGGER `xmppmaster`.`up_black_list_BEFORE_INSERT` BEFORE INSERT ON `up_black_list` FOR EACH ROW
BEGIN
