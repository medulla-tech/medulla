...
class class01testMirror(unittest.TestCase):
    ...
    def test102getFilePath(self):
        ...
        self.assertEqual(
            result, "%s://%s:9990/mirror1_files/test/MD5SUMS" % (protocol, ipserver)
        )
    ...
    def test104getFilesURI(self):
        result = serverM.getFilesURI(
            ["5d3ff03e396aa072f5cae2b2ddcd88b9", "7885517b39317add6a1d362968b01774"]
        )
        self.assertEqual(
            result,
            [
                "%s://%s:9990/mirror1_files/test/MD5SUMS" % (protocol, ipserver),
                "%s://%s:9990/mirror1_files/test/install.bat" % (protocol, ipserver),
            ],
        )
                    ...
                    "files": [
                        {
                            "path": "/test",
                            "name": "install.bat",
                            "id": "7885517b39317add6a1d362968b01774",
                        },
                        {
                            "path": "/test",
                            "name": "MD5SUMS",
                            "id": "5d3ff03e396aa072f5cae2b2ddcd88b9",
                        },
                    ],
                    "installInit": {"command": "", "name": ""},
                    "description": "Ceci est le package de test",
                    "preCommand": {"command": "", "name": ""},
                    "basepath": direct,
                    "reboot": "1",
                    "label": "TestPackage",
                    "version": "2.0.0.9",
                    "command": {"command": "./install.bat", "name": "commande"},
...
class class05valid_urlTest(unittest.TestCase):
    ...
    def test501valid_url(self):
        for package in serverM.getFilesURI(
            ["7885517b39317add6a1d362968b01774", "5d3ff03e396aa072f5cae2b2ddcd88b9"]
        ):
...
if makefile:
    ...
    system(
        "sed 's/src = /src = \/tmp\/%s/' package_server.conf > package-server.conf"
        % (directory_temp[5:])
    )
...
if mode == "debug":
    ...
    for klass in [
        ...
        class05valid_urlTest,
        class06removedirTest,
    ]:
