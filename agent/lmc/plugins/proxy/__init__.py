...
class ProxyConfig(PluginConfig):
    ...
    def check(self):
        ...
        cmd = lmctools.shLaunch(self.sgBinary + " -v")
        if cmd.exitCode:
...
class Blacklist:
    ...
    def restartSquid(self):
        ...
        """
        lmctools.shlaunch(self.config.sgBinary + ' -C ' + self.config.sgBlacklist)
        lmctools.shlaunch("chown " + self.config.squidGroup + "." + self.config.squidUser + " " + self.config.sgBlacklist + "*")
