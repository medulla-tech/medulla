...
class AuditReaderDB:
    ...
    def getLog(self, start, end, plug, user, type, date1, date2, object, action):
        ...
        if (date1 != 0) and (date2 != 0):
            ql = qlog.filter(
                and_(
                    self.parent.record_table.c.date >= date1,
                    self.parent.record_table.c.date <= date2,
                )
            )
            qlog = ql
        elif date1 != 0:
            ...
            if obj != []:
                ...
                if object_log:
                    ...
                    for idobjectlog in object_log:
                        orobjlog = or_(
                            orobjlog,
                            self.parent.record_table.c.id == idobjectlog.record_id,
                        )
        ...
        if type != 0:
            ...
            if typ != None:
                ql = qlog.filter(self.parent.object_table.c.type_id == typ.id).join(
                    "obj_log"
                )
                qlog = ql
            else:
        ...
        return [
            logcount,
            self.getArrayLogs(
                qlog.order_by(desc(self.parent.record_table.c.id))
                .limit(end - start)
                .offset(start)
            ),
        ]
    ...
    def getArrayLogs(self, qlog):
        ...
        for record in qlog:
            laction = (
                ...
                .filter(
                    and_(
                        self.parent.event_table.c.id == record.event_id,
                        self.parent.event_table.c.module_id == record.module_id,
                    )
                )
                .first()
            )
            lparam = (
                ...
                .filter(self.parent.param_table.c.record_id == record.id)
                .all()
            )
            lplugin = (
                self.session.query(Module)
                .filter(self.parent.module_table.c.id == record.module_id)
                .first()
            )
            lclient = (
                self.session.query(Initiator)
                .filter(self.parent.initiator_table.c.id == record.initiator_id)
                .first()
            )
            listobj = (
                self.session.query(Object_Log)
                .filter(self.parent.object_log_table.c.record_id == record.id)
                .all()
            )
            luser = (
                self.session.query(Object)
                .filter(self.parent.object_table.c.id == record.user_id)
                .first()
            )
            lagent = (
                self.session.query(Source)
                .filter(self.parent.source_table.c.id == record.source_id)
                .first()
            )
            ...
            if lparam != None:
                ...
                for param in lparam:
                    parameters.__setitem__(
                        str(param.param_name), str(param.param_value)
                    )
            ...
            if listobj != None:
                for objects in listobj:
                    ...
                    ltype = (
                        self.session.query(Type)
                        .filter(self.parent.type_table.c.id == lobject.type_id)
                        .first()
                    )
                    ...
                    lpattr = (
                        self.session.query(Previous_Value)
                        .filter(
                            self.parent.previous_value_table.c.object_log_id
                            == objects.id
                        )
                        .all()
                    )
                    ...
                    lcattr = (
                        ...
                        .filter(
                            self.parent.current_value_table.c.object_log_id
                            == objects.id
                        )
                        .all()
                    )
